...
export const SHELF_INTROS: Partial<Record<CompendiumSectionId, CompendiumEntry>> = {
  ...
  order: lead('order', 'About Orders', '❧', [
    ...
    'An Order does nothing while it sits in your collection. You place it in one of your government’s slots on the Statecraft screen, and it works for exactly as long as it stays there. Slots are military, economic or wildcard; a card fits a slot of its own kind, and a wildcard slot takes any card. A newly placed Order is sealed for a few turns, meaning it cannot be moved yet.',
    'Some words on these cards are the game’s own. Your writ is your authority, the meter that limits how many cities you can govern; a torn writ means authority has gone negative. A camp is a barbarian camp. Chopping is clearing a forest or jungle with a worker. A discovery is the ruin a unit claims by walking onto it. A frontier city is one with another empire’s territory nearby. A garrison is the units standing inside a city. Border culture is the culture a city puts toward claiming its next hex, which is banked separately from the culture that earns you drafts. Overflow is the production left over when something finishes. A unique luxury means each different kind of luxury resource you have improved, however many copies of it you hold.',
