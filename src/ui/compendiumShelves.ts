...
export const SHELF_INTROS: Partial<Record<CompendiumSectionId, CompendiumEntry>> = {
  ...
  bead: lead('bead', 'About the Bead Race', '◉', [
    ...
    'There are four kinds. A feat is a first in the world — the first religion founded, the first empire into a new age — and it is always in play, whether or not anybody is looking for it. A race project is a card that puts a row in every empire’s build list; the first empire to finish it takes the bead and the reward, and everyone else’s production is spent for nothing. A quest is a deed you go and do. A reckoning is the age taking a measurement of everybody at once — most cities, largest city, most technologies — and naming one winner; a tie pays nobody.',
    'The last three are dealt. Each age has its own deck, shuffled when the world was made, so the seed decides the deal and every player sees the same cards. One card is dealt to the table each turn, face down until the first empire in the world reaches that age, at which point the whole table turns over at once. A card that somebody claims leaves the table, and the deck fills the gap.',
    ...
    'One slot on every rod is different. The last bead is golden, it can only be earned by completing the Magnum Opus, and its slot sits empty from the first turn — the standing question of the whole game. The pages below give the rules of the race, then every feat, race project, quest and reckoning in it.',
  ]),
