...
export const CONCEPT_ENTRIES: readonly CompendiumEntry[] = [
  ...
  written(
    ...
    [
      ...
      'Details: enemy units and cities exert zone of control on the hexes next to them — moving from one hex next to an enemy to another hex next to the same enemy ends your movement. Combat is a single attack; both strengths are modified by terrain, fortification, garrison, cards and wonders. A melee unit that destroys the defender advances into its hex. A civilian unit alone on a hex is captured by a melee attack, except a trader, which is destroyed and its cargo taken. A camp you have seen stays on your map until you see the hex again without it.',
    ],
  ),
  written(
    'concept',
