...
export const CONCEPT_ENTRIES: readonly CompendiumEntry[] = [
  ...
  written(
    ...
    '⇄',
    [
      'Trade is a way to make your cities richer and to build roads at the same time. Once you have researched Currency you can build a trader, a civilian unit. Select a trader and choose Start route. Pick any available route in the Trade screen; the trader moves to the origin city and begins. The route pays extra food, production and gold to the destination city every turn.',
  ...
  written(
    ...
    [
      ...
      'Details: enemy units and cities exert zone of control on the hexes next to them — moving from one hex next to an enemy to another hex next to the same enemy ends your movement. Combat is a single attack. Hills and forests add to a defender\'s strength, and fortifying adds more each turn it stays, up to a cap; cards and wonders can add further bonuses to either side. A city takes no benefit from the ground it stands on — instead it defends with the strength of the best unit its owner could build, plus its walls. A city with every neighbouring land hex controlled by enemies is under siege: it cannot heal and loses a little health each turn, but only an attack can capture it. A melee unit that destroys the defender advances into its hex. A civilian unit alone on a hex is captured by a melee attack, except a trader, which is destroyed and its cargo taken. A camp you have seen stays on your map until you see the hex again without it.',
    ],
  ),
  written(
    'concept',
