...
export const INTRO_ENTRIES: readonly CompendiumEntry[] = [
  ...
  written(
    ...
    [
      ...
      'Details: barbarians are not a civilisation and do not count — you do not have to clear the map of them to win. A game with only one player never declares a winner. The other way to win is the Bead Race: a race to earn beads, scored on the Abacus screen. Beads open the door rather than settle the game — fill your rod and you may begin the Magnum Opus, the final project; finishing the Great Work wins the game outright, however many beads anybody else holds.',
    ],
  ),
  written(
    'intro',
...
export const CONCEPT_ENTRIES: readonly CompendiumEntry[] = [
  ...
  written(
    ...
    [
      ...
      'A religion holds beliefs of its own, drawn with a prophet\'s charges, and the two pools are paid to two different people. A follower belief applies in every city that follows the faith and pays whoever owns that city — yours or a rival\'s, which is why a rival\'s faith taking hold in your towns is a gift rather than a wound. An enhancer belief bends the spread itself, and it pays whoever holds the faith\'s holy city: the city whose land the first holy site stands on. So does the trickle that foreign following cities pay in faith and gold. Lose that city to a conqueror and the trickle and the enhancers go with it; your pantheon, which is native to your empire, does not.',
      'A prophet carries charges, and its acts do not all cost the same. Founding your religion where it stands, raising the holy site that anchors it, takes every charge the prophet has, and so does drawing another belief for a religion you have already founded — those are the acts that settle what your faith is, and a prophet that does one of them is gone. Once your faith stands, a prophet may instead raise a further holy site on land of your own: the faith spreads from the new stones as it does from the old, though the seat of the faith stays where it was founded and no belief comes with it. Speaking costs less: a proclamation, which is a wide, strong pulse of conversion on the hex it stands on that converts and then fades to nothing, leaving no site behind; or a rite said over every city you own at once, for a single price in faith. So a prophet either founds or deepens your religion and leaves the board, or it speaks twice and then leaves it. The sheet on the piece prints what each act costs, and greys the ones it can no longer afford.',
      'Details: each augur and each prophet costs more faith than the last, and they are counted separately. An apostle and an inquisitor spend one charge on each of their acts, so an apostle speaks or heals or leaves its relic twice and an inquisitor purges once. The holy site is the only thing about a religion an enemy can take away, by capturing the hex or pillaging it. The pantheon is never redrafted — it is what the religion was founded out of. A captured city keeps its followers, exactly as it keeps its buildings. Your religion, its beliefs, what its holy city is paid, and every city in the world that follows are all on the Religion screen; a city\'s own congregations are on its city screen, and hovering one shows what is pressing on it and from where.',
    ],
  ),
  written(
    'concept',
    ...
    '⇄',
    [
      'Trade is a way to make your cities richer and to build roads at the same time. Once you have researched Currency you can hire trade routes with gold on the Trade screen. Pick a pair of your cities — or one of yours and a neighbour\'s — and pay for the route; a caravan appears in the origin city and sets out at once. A caravan is its own kind of unit, neither a soldier nor a civilian, with a slot of its own on a hex. The route pays extra food, production and gold to the destination city every turn.',
      'Where you send it matters: a route pays the destination city based on the origin city\'s buildings, so run routes out of your best-developed cities to the ones that need feeding. A route by land wears a road into every hex the caravan passes, and roads make every unit move faster; a route by sea lays no road and pays more for the crossing instead. Open the Trade screen from the cart on the top bar or the cart beside the other sheets, and it will recommend the routes worth sending: the ones that pay most, the ones that finish a road to a town the capital cannot reach, the ones that feed a town still growing, and the ones abroad that teach you the most.',
      'Details: each market provides one route slot; the top bar shows routes in use and available. With no slot free, the Trade screen reads "Not enough trade route capacity" over the routes it cannot send, and every other route you cannot send today is kept on the same tab with the reason for it. A route lasts a fixed number of turns and can be set to renew automatically, and you can end one early to free its slot. Roads are permanent and any unit can use them: moving between two road hexes costs a fraction of a movement point and ignores terrain. Each city connected to your capital by road pays gold every turn; the roads you built cost a small upkeep. An enemy melee unit can attack a trader and take its cargo.',
    ],
  ),
  written(
    'concept',
  ...
  written(
    ...
    [
      ...
      'Barbarians appear from camps in unexplored land and raid nearby cities. A camp keeps one of its units at home, dug in on the camp itself, and sends the rest out — so clearing a camp usually means beating its guard first. Clearing a camp with a military unit gives a reward to your nearest city. Keep a warrior near each new city early on.',
      'Details: enemy units and cities exert zone of control on the hexes next to them — moving from one hex next to an enemy to another hex next to the same enemy costs one extra movement point. The general\'s bonus reaches a short distance, applies to your military units only, and does not stack with a second general. Combat is a single attack. Hills and forests add to a defender\'s strength, and fortifying adds more each turn it stays, up to a cap; cards and wonders can add further bonuses to either side. A city takes no benefit from the ground it stands on — instead it defends with the strength of the best unit its owner could build, plus its walls. A city with every neighbouring land hex controlled by enemies is under siege — but only once the besieger has learned Siegecraft: before that technology, war is raids, and a surrounded city still heals. Under a real siege it cannot heal and loses a little health each turn, but only an attack can capture it. Once a city\'s defence is beaten down, a melee unit that kills the last defender standing in it takes the city in the same blow; a ranged unit that kills the defender leaves the city to be walked into. A melee unit that destroys the defender advances into its hex. Fighting across a shoreline has rules of its own: a melee unit cannot attack anything on water at all, and a unit crossing water cannot attack a ship, though it can still attack the shore. Archers do reduced damage to ships and siege engines do increased damage, and a ship attacking a unit crossing water takes only part of the blow back. A civilian unit alone on a hex is captured by a melee attack, except a trader, which is destroyed and its cargo taken. A camp you have seen stays on your map until you see the hex again without it.',
    ],
  ),
  written(
    'concept',
