/**
 * The two shaders the whole prototype is built out of: a banded toon material
 * and an inverted-hull outline material.
 *
 * Toon banding
 * ------------
 * `MeshToonMaterial` looks up `dot(N, L)` in a 1D `gradientMap` instead of
 * using it directly, so the ramp texture *is* the lighting model. Ours is an
 * N-pixel `DataTexture` with `NearestFilter`, which is the entire trick: three
 * pixels, no interpolation, three flat bands of light on every surface.
 *
 ...
 * along the surface normal, in near-black. From the camera the expanded shell
 * is hidden behind the real mesh everywhere except at the silhouette, where it
 * spills out as a rim.
 *
...
import { saturate } from './palette';

/** How dark the bottom band of the ramp is allowed to get. */
 ...
 */
export function makeGradientMap(steps: number): DataTexture {
  const n = Math.max(1, Math.round(steps));
  const data = new Uint8Array(n);
  for (let i = 0; i < n; i++) {
    const t = n === 1 ? 1 : i / (n - 1);
    data[i] = Math.round((RAMP_FLOOR + (1 - RAMP_FLOOR) * t) * 255);
  }
...
export interface ToonOptions {
  /** Transparency for water. Omitted means fully opaque. */
  opacity?: number;
}

/**
 * Owns every material in the scene so the look-dev sliders have one place to
 * write to. Materials are cached by `(colour, opacity)`, which is also the
 * instancing key — two kinds of geometry sharing a colour share a material, and
 * the InstancedMesh split falls out of the geometry alone.
 */
export class MaterialLibrary {
  ...
  constructor(rampSteps: number, outlineColor: number) {
    this.gradientMap = makeGradientMap(rampSteps);
  ...
  get(color: number, options: ToonOptions = {}): MeshToonMaterial {
    ...
    const material = new MeshToonMaterial({
      ...
      // not requested here: `MeshToonMaterial` does not implement
      // `flatShading`, and setting it would be silently dropped.
  ...
  setRampSteps(steps: number): void {
    const next = makeGradientMap(steps);
