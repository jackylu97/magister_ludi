...
export class Renderer3D implements MapView {
  ...
  /**
   * One frame's colour pass, attributed to the batch that submitted it (#18).
   *
   * `benchmarkProbe` counts a frame whole, and the overview's question is
   * *which* family fills it. The only honest answer is the draw list the
   * renderer really walked — after frustum culling, after the board's own LOD
   * visibility — so the tally hangs off `onBeforeRender`, which three calls once
   * per object it actually draws, and divides by the passes
   * `Scene.onBeforeRender` counted. Shadow depth goes through `onBeforeShadow`
   * and is deliberately outside this: the static sun bakes on world changes,
   * not on the frames sampled here.
   *
   * Its own window, on plain animation frames rather than through the probe: a
   * closure call per draw is a cost the phase timings must not carry.
   */
  private async sampleColourPass(frames: number): Promise<{family: string; draws: number; triangles: number; instances: number}[]> {
    ...
    scene.traverse((object) => {
      ...
      if (!family) {
        // Anything outside the board answers for its layer: a name the renderer
        // already gave the group when it added it to the scene.
        let node = object;
    ...
    return [...tally.values()].map(row => ({family: row.family, draws: row.draws / per,
  ...
  async benchmarkTerrain(): Promise<unknown> {
    ...
    let workloads: unknown = null, colourPass: unknown = null;
    try {
      // The parked omniscient overview leads, and it stands still: a camera
      // that pans cannot be screenshotted twice the same way, and this is the
      // view the far LOD is answerable to. Its warm-up absorbs the seat change
      // and the detail flip; the attribution takes its own window afterwards,
      // because its hooks cost a closure call on every draw of every frame.
      if (options.views) {
        this.setFogSeat(null);
        this.view.frameBoard(this.board.bounds);
        this.view.panTo(map.width*Math.sqrt(3)*.5, map.height*.75, false, performance.now());
        probe.phase('parked omniscient overview', 'camera parked on the whole board with the seat dropped; a redraw forced every frame');
        ...
        probe.count(false);
        colourPass = await this.sampleColourPass(Math.max(8, Math.round(options.frames / 6)));
      }
      ...
      return {
        ...
        colourPassAttribution: colourPass,
        workloads,
      };
    } finally {
