...
export interface PaintedLodSpec {
  ...
  farPixels: number;
  /**
   * The grid a prop family's sculpt is clustered on to make the map-scale
   * stand-in the far batches draw (`farSculpt`, `terrainStudy/vegetation.js`):
   * coarser is cheaper and blunter, and a family with no entry here draws its
   * authored sculpt at every distance. Near geometry, picking and the static
   * shadow bake never read this — they are always the sculpt.
   */
