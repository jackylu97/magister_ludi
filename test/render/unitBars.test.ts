...
import { buildHpBar, hpBarFill } from '../../src/render3d/pieces';
...
describe('a piece keeps its bar while it walks', () => {
  ...
  it('hangs the two quads where the resting bar hangs them', () => {
    ...
    // Backing **first**: neither quad tests depth, so what draws the fill in
    // front is the order the two were built in. `pieces3d.test.ts` pins the same
    // guarantee for the instanced pair.
    expect(backing.id).toBeLessThan(fill.id);
    for (const quad of quads) expect(quad.renderOrder).toBe(RENDER_ORDER.hpBar);
