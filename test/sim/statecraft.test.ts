...
import { explainCardBuildingYields } from "../../src/sim/yields/town";
...
describe("the card table", () => {
  ...
  it("opens on the one tier-0 government, and offers a fixed triple at each other tier", () => {
    ...
    expect(
      GOVERNMENT_IDS.filter((id) => governmentDef(id).tier === 0),
    ).toHaveLength(1);
    ...
    expect(STATECRAFT.tierLadder.slice(0, GOVERNMENT_TIERS.length)).toEqual([
      ...GOVERNMENT_TIERS,
    ]);
  ...
  it("grows the slot spread monotonically up the ladder", () => {
    ...
    for (const tier of GOVERNMENT_TIERS) {
      ...
      last = Math.min(
        ...governmentsAtTier(tier).map((id) => slotLayout(id).length),
      );
    }
  });
  ...
  it("lays every pool out with enough cards to draft from, of every slot type", () => {
    for (const pool of ORDER_POOLS) {
      ...
      for (const type of SLOT_TYPES) {
        expect(
          cards.some((id) => orderDef(id).slot === type),
          `${pool}/${type}`,
        ).toBe(true);
      }
    }
  ...
  it("fills every live Doctrine pool, and keeps the deferred one out of all of them", () => {
    ...
    const dealt = new Set(
      GOVERNMENT_TIERS.flatMap((tier) => poolDoctrines(tier)),
    );
  ...
  it("opens with a slot for every kind of card the chiefdom pool can deal", () => {
    ...
    for (const id of pool) {
      expect(
        layout.includes(orderDef(id).slot) || layout.includes("wildcard"),
        id,
      ).toBe(true);
    }
  });
  ...
  it("states the two tier-3 rivals in the words they were ratified in", () => {
    ...
    expect(
      describeCard("councilOfElders").map((clause) => clause.text),
    ).toEqual(["+3 happiness", "+1 renown per turn in every city"]);
  ...
  it("gives every card a name, a flavour line and at least one effect or a stated deferral", () => {
    for (const id of [...GOVERNMENT_IDS, ...DOCTRINE_IDS, ...ORDER_IDS]) {
      ...
      if (
        ORDER_IDS.includes(id as never) &&
        orderDef(id as never).retired === true
      )
        continue;
      ...
      const granted = ORDER_IDS.includes(id as never)
        ? (orderDef(id as never).onSlot ?? [])
        : [];
      const hasSomething =
        def.effects.length > 0 ||
        (def.deferred ?? []).length > 0 ||
        granted.length > 0;
  ...
  it("reads every effect kind in the union from at least one live card", () => {
    ...
    const walk = (
      effects: readonly { kind: CardEffectKind; then?: unknown }[],
    ): void => {
  ...
  it("reads every (where, basis) pair of the pays shape from a live row", () => {
    ...
    const walk = (effects: readonly CardEffect[]): void => {
      for (const effect of effects) {
        if (effect.kind === "pays")
          pairs.add(`${effect.where}/${effect.basis ?? "flat"}`);
        const nested = (effect as { then?: CardEffect[] }).then;
        if (nested) walk(nested);
      }
    };
    for (const id of [...GOVERNMENT_IDS, ...DOCTRINE_IDS, ...ORDER_IDS])
      walk(cardDef(id).effects);
...
describe("the command matrix", () => {
  ...
  it("refuses a pick with no draft outstanding", () => {
    ...
    refuses(
      g,
      { type: "chooseOrder", playerId: 0, optionIndex: 0 } as Command,
      "no Statecraft draft",
    );
  });
  ...
  it("refuses an option that was not dealt, and a non-integer index", () => {
    ...
    refuses(
      g,
      { type: "chooseOrder", playerId: 0, optionIndex: 9 } as Command,
      "not one of the",
    );
    refuses(
      g,
      { type: "chooseOrder", playerId: 0, optionIndex: -1 } as Command,
      "not one of the",
    );
    refuses(
      g,
      { type: "chooseOrder", playerId: 0, optionIndex: 1.5 } as Command,
      "integer optionIndex",
    );
  });
  ...
  it("refuses a type mismatch with the reducer’s own sentence", () => {
    ...
    expect(slotLayout(sc.government)).toEqual([
      "military",
      "economic",
      "wildcard",
    ]);
  ...
  it("refuses every Statecraft verb from a seat that has ended its turn", () => {
    ...
    for (const [command, fragment] of [
      [
        { type: "chooseOrder", playerId: 0, optionIndex: 0 },
        "cannot choose an Order",
      ],
      [
        {
          type: "slotOrder",
      ...
      [
        { type: "adoptGovernment", playerId: 0, choiceIndex: 0 },
        "cannot adopt",
      ],
      [
        { type: "chooseDoctrine", playerId: 0, optionIndex: 0 },
        "cannot choose a Doctrine",
      ],
    ] as [Command, string][]) {
  ...
  it("refuses an adoption with nothing banked, and a Doctrine with no draft", () => {
    ...
    refuses(
      g,
      { type: "adoptGovernment", playerId: 0, choiceIndex: 0 } as Command,
      "no government offer",
    );
    refuses(
      g,
      { type: "chooseDoctrine", playerId: 0, optionIndex: 0 } as Command,
      "no Doctrine draft",
    );
  });
  ...
  it("agrees with its own error functions", () => {
    ...
    expect(slotOrderError(g.state, 0, "firstRites", 0)).toContain(
      "does not hold",
    );
    expect(unslotOrderError(g.state, 0, 0)).toContain("is empty");
    expect(slotOrderError(g.state, 0, "notACard", 0)).toContain(
      "not a known Order",
    );
  });
});
...
describe("seals", () => {
  it("seals a card on entry, for the empire’s own length", () => {
    ...
    expect(sealRemaining(g.state, sc.slots[0]!)).toBe(
      STATECRAFT.meter.sealTurns,
    );
  });
  ...
  it("refuses to unslot a sealed card, and allows it the turn the seal lifts", () => {
    ...
    expect(
      dispatch(g, { type: "unslotOrder", playerId: 0, slotIndex: 0 } as Command)
        .ok,
    ).toBe(true);
...
describe("adoption", () => {
  ...
  it("opens the next Order pool on adoption", () => {
    ...
    expect([...pool].some((id) => orderDef(id).pool === "governmentI")).toBe(
      true,
    );
    expect([...pool].every((id) => orderDef(id).pool === "governmentI")).toBe(
      true,
    );
  });
  ...
  it("never offers a Doctrine twice in one game", () => {
    ...
    expect(new Set(player.statecraft.doctrines).size).toBe(
      GOVERNMENT_TIERS.length,
    );
  });
});
...
describe("every hook family, end to end", () => {
  it("tileYield — The Orchard Tithe is a line in the hex breakdown", () => {
    ...
    expect(explainTileYield(bare, yieldContextFor(g.state, 0))).toHaveLength(
      before.length - 1,
    );
  });
  ...
  it("percentYields — a card joins the city stage rather than multiplying afterwards", () => {
    ...
    const food = lines.filter(
      (line) => line.yield === "food" && line.source.startsWith("Doctrine"),
    );
  ...
  it("productionBonus — Conscription is a labelled modifier line", () => {
    ...
    const building = productionModifiers(g.state, city, {
      kind: "building",
      id: "granary",
    });
    expect(building.some((line) => line.source.includes("Conscription"))).toBe(
      false,
    );
  });
  ...
  it("happiness and authority — cards are lines in both ledgers", () => {
    ...
    expect(
      explainHappiness(g.state, 0).some((l) =>
        l.source.startsWith("Order · Festival Days"),
      ),
    ).toBe(true);
  ...
  it("countScaled — Salt Tithes scales with what the empire holds, capped where capped", () => {
    ...
    expect(
      explainCardEmpireYields(g.state, 0).some((l) => l.card === "saltTithes"),
    ).toBe(false);
  });
  ...
  it("rateConversion — The Tithe reads the turn’s rate, not the bank", () => {
    ...
    bumpRevision(g.state);
    expect(
      explainCardEmpireYields(g.state, 0, { faithPerTurn: 7 }).find(
        (l) => l.card === "theTithe",
      )?.gold,
    ).toBe(7);
    // Zero rate, no line — a card that pays nothing is not in the list.
    expect(
      explainCardEmpireYields(g.state, 0, { faithPerTurn: 0 }).some(
        (l) => l.card === "theTithe",
      ),
    ).toBe(false);
  });
  ...
  it("windfallRider — The Woodwrights changes the printed number", () => {
    ...
    expect(payout.grants).toEqual([
      {
        card: "woodwrights",
        source: "Doctrine · The Woodwrights",
        yield: "culture",
        amount: 10,
      },
    ]);
  ...
  it("foundingRider — The Founders’ Road founds no hall, and pays every town its culture", () => {
    ...
    const line = explainCardCityYields(g.state, city).find(
      (l) => l.card === "foundersRoad",
    )!;
    ...
    expect(
      explainCardCityYields(g.state, second).find(
        (l) => l.card === "foundersRoad",
      )!.culture,
    ).toBe(1);
  });
  ...
  it("conditionRule — The Hermit Crown opens and closes with the city count", () => {
    ...
    bumpRevision(g.state);
    const open = cityYieldPercents(g.state, city).filter((l) =>
      l.source.includes("Hermit"),
    );
    // `yield: 'all'` expands into one labelled line per voice.
    expect(open).toHaveLength(6);
    expect(open.every((l) => l.percent === 30 && l.stage === "city")).toBe(
      true,
    );
    ...
    bumpRevision(g.state);
    expect(
      cityYieldPercents(g.state, city).some((l) => l.source.includes("Hermit")),
    ).toBe(true);
  ...
  it("meterRule — Hegemony reprices a captured city in the writ ledger", () => {
    ...
    void city;
    const before = explainAuthority(g.state, 0).find((l) =>
      l.source.includes("captured"),
    )!;
    g.state.players[0]!.statecraft.doctrines.push("hegemony");
    bumpRevision(g.state);
    const after = explainAuthority(g.state, 0).find((l) =>
      l.source.includes("captured"),
    )!;
...
describe("determinism", () => {
  it("round-trips a schema 40 save with Statecraft in it", () => {
    ...
    expect(JSON.parse(text).players[1].statecraft).toEqual(
      newPlayerStatecraft(),
    );
  });
...
describe("rule 5 holds with cards active", () => {
  ...
  it("both meters stay the fold of their own ledgers", () => {
    ...
    expect(happinessOf(g.state, 0)).toBe(
      foldMeter(explainHappiness(g.state, 0)),
    );
    expect(authorityOf(g.state, 0)).toBe(
      foldMeter(explainAuthority(g.state, 0)),
    );
  });
  ...
  it("a tile’s total stays the fold of its own breakdown", () => {
    ...
    expect(foldTile(tile, ctx)).toEqual(
      foldTileLines(explainTileYield(tile, ctx)),
    );
  });
  ...
  it("a windfall’s printed number is the fold of its own riders", () => {
    ...
    expect(
      windfallPayout(g.state, 0, "camp").grants.map((g2) => g2.yield),
    ).toEqual(["food"]);
  });
  ...
  it("perSlottedOrder composes with perAge as a product, not as a second percentage", () => {
    ...
    const rider = governmentDef("warChief").effects.find(
      (effect) =>
        effect.kind === "windfallRider" && effect.grant?.yield === "science",
  ...
  it("perSlottedOrder pays into the empire’s banks, and only what it printed", () => {
    ...
    const before = snapshotState(g.state);
    expect(windfallPayout(g.state, 0, "kill")).toEqual(
      windfallPayout(g.state, 0, "kill"),
    );
    expect(snapshotState(g.state)).toEqual(before);
...
describe("the behavioural hooks, in the verbs they change", () => {
  ...
  it("windfallRider chop — the timber banked is the printed number, riders folded in", () => {
    const plain = (doctrine?: "woodwrights") => {
      ...
      chopFeatureAt(g.state, worker, tile);
      return {
        hammers: city.hammerBasket,
        culture: g.state.players[0]!.culturePool - cultureBefore,
      };
    };
    const bare = plain();
  ...
  it("windfallRider pillage — the salvage, the gold rider and the heal all land", () => {
    ...
    expect(g.state.players[0]!.gold).toBe(
      gold + RULES.improvements.pillageGold + 10,
    );
  ...
  it("periodicMuster — The Standing Levy raises a spear on the calendar, not on an occasion", () => {
    ...
    const mine = (): number =>
      g.state.units.filter((u) => u.ownerId === 0).length;
    ...
    expect(other.state.units.filter((u) => u.ownerId === 0).length).toBe(
      before,
    );
  });
  ...
  it("effectAmplifier — The Grand Bazaar reaches into the luxury table", () => {
    ...
    const seam = ownedTiles(g.state, city).find(
      (tile) => tile.col !== city.col,
    )!;
    seam.resource = "silk";
    seam.improvement = "plantation";
    const before = explainHappiness(g.state, 0).find((line) =>
      line.source.startsWith("Silk"),
    );
    expect(before).toBeDefined();
    g.state.players[0]!.statecraft.doctrines.push("grandBazaar");
    bumpRevision(g.state);
    const after = explainHappiness(g.state, 0).find((line) =>
      line.source.startsWith("Silk"),
    );
  ...
  it("metaRule and cityStat reach the two rules nothing else can", () => {
    ...
    const citySight = (): number => {
      const sources = sightSources(g.state, 0).filter(
        (source) =>
          source.tile.col === city.col && source.tile.row === city.row,
      );
...
describe("a card is what it prints", () => {
  it("carries no level, no ceiling and no second face anywhere in the table", () => {
    ...
    expect(
      (STATECRAFT as unknown as Record<string, unknown>).maxOrderLevel,
    ).toBeUndefined();
  });
...
describe("the draw is weighted by rarity", () => {
  ...
  it("deals a rare card about a thirteenth as often as three commons", () => {
    ...
    // Fifteen of a hundred and thirty, on seed 5, exactly — a number this build
    ...
    // until schema 102 and twelve until 107, and both moves are the same one
    // move: `newGame` used to shuffle two decks of deed cards off this
    // generator, G2 took four reckonings out of each of them, and Q1 took the
    // decks themselves — so every draw in the game after `newGame` starts from a
    // different place. The *frequency* is unchanged: it is the same weighted
    // bag, and 15/130 is 0.115 against a weight share of 1/13 = 0.077 on a
    // sample this small.
    expect(rares).toBe(15);
  });
  ...
  it("keeps the military/economic/wildcard spread under weighting", () => {
    ...
    const sc = g.state.players[0]!.statecraft;
    for (let i = 0; i < 10; i++) {
      const hand = drawOrderOptions(g.state, livePool(sc), 3, (id) =>
        orderDrawWeight(id, 0),
      );
      expect(hand, `draft ${i}`).toHaveLength(3);
      expect(
        new Set(hand.map((id) => orderDef(id).slot)),
        `draft ${i}`,
      ).toEqual(new Set(SLOT_TYPES));
    }
  });
  ...
  it("deals a hand the pity has changed, from the same state", () => {
    ...
    // The seed is chosen rather than arbitrary: a weighted draw with a heavier
    // thumb on it may still land on the same three cards, so the pin needs a
    // seed where it does not — 23 was that seed until batch C1 moved it to 22 and batch Q1 moved the whole
    // stream (schema 108) and 31 is it now.
    const clean = game(31);
    const patient = game(31);
...
describe("passing on a draft", () => {
  it("spends the hand, raises the pity, and blocks nothing afterwards", () => {
    ...
    const sc = player.statecraft;
    sc.pendingOrder = {
      options: ["firstRites", "saltTithes", "bloodedSpears"],
    };
    expect(statecraftBlocker(player)).toBe("an Order draft is waiting");
    expect(
      dispatch(g, { type: "skipOrderOffer", playerId: 0 } as Command).ok,
    ).toBe(true);
  ...
  it("refuses a pass with no draft on the table, byte-identically", () => {
    ...
    expect(refused.ok).toBe(false);
    if (!refused.ok)
      expect(refused.error).toContain("no Statecraft draft to pass on");
    expect(snapshotState(g.state)).toEqual(before);
  });
...
describe("the master-list cut of 2026-08-28", () => {
  ...
  it("routeYields — the charter is a line in the caravan breakdown, not a multiplication after it", () => {
    ...
    expect(paid.production).toBe(
      before.production + Math.floor((before.production * 50) / 100),
    );
  });
  ...
  it("greatPeopleOfFamily — The Empire scales with the generals it has earned", () => {
    ...
    expect(before.bonuses.some((b) => b.source.includes("The Empire"))).toBe(
      false,
    );
    ...
    bumpRevision(g.state);
    const third = previewCombat(g.state, mine.id, {
      col: target.col,
      row: target.row,
    });
    if (third.ok) {
  ...
  it("discoveredCamps and vsBarbarians — Border Ballads pays for the wild twice over", () => {
    ...
    expect(
      explainCardEmpireYields(g.state, 0).some(
        (l) => l.card === "borderBallads",
      ),
    ).toBe(false);
    ...
    expect(
      windfallPayout(g.state, 0, "kill", 0, 0, { vsBarbarians: true }).grants,
    ).toEqual([
      {
        card: "borderBallads",
        source: "Order · Border Ballads",
        yield: "culture",
        amount: 10,
      },
    ]);
  ...
  it("randomMilitary — Camp Followers gifts a piece, and the same piece on a replay", () => {
    const draw = (seed: number): { type: string; note: string } => {
      ...
      return {
        type: payout.units[0]!.type,
        note: payout.lines.map((l) => l.note).join("|"),
      };
    };
    const first = draw(7);
    ...
    const payout = windfallPayout(g.state, 0, "camp");
    payWindfallGrants(g.state, playerById(g.state, 0)!, payout, {
      col: city.col,
      row: city.row,
    });
  ...
  it("population within a city — the Republic pays each town for its own citizens", () => {
    ...
    expect(
      explainCardEmpireYields(g.state, 0).some((l) => l.card === "republic"),
    ).toBe(false);
  });
  ...
  it("unlocksBuilding — the Gilded Hall is opened by the Court and sold, never built", () => {
    ...
    expect(buildError(g.state, 0, "building", "gildedHall", city)).toContain(
      "bought",
    );
    player.gold = 10000;
    expect(
  ...
  it("barbarianKillsConvert — Wolf-Mother's Pact takes the fallen instead of burying them", () => {
    ...
    raider.hp = 1;
    const preview = previewCombat(g.state, mine.id, {
      col: target.col,
      row: target.row,
    });
  ...
  it("offers Gov IV and Gov V at their rungs, and deals a Doctrine pool for each", () => {
    for (const tier of [29, 45]) {
      ...
      expect(poolDoctrines(tier).length).toBeGreaterThanOrEqual(
        RULES.offers.doctrine,
      );
      ...
      for (const id of governmentsAtTier(tier)) {
        expect(poolOfGovernment(id)).toBe(
          tier === 29 ? "governmentIV" : "governmentV",
        );
      }
    }
    const g = game();
    ...
    expect(player.statecraft.pendingGovernment?.options).toEqual(
      governmentsAtTier(29),
    );
    expect(
      dispatch(g, {
        type: "adoptGovernment",
    ...
    expect(player.statecraft.pendingDoctrine?.options.length).toBe(
      RULES.offers.doctrine,
    );
    for (const id of player.statecraft.pendingDoctrine!.options) {
  ...
  it("retires The Loose Rein from every pool while keeping the row readable", () => {
    ...
    for (const pool of ORDER_POOLS)
      expect(poolOrders(pool)).not.toContain("theLooseRein");
  ...
  it("prints every changed row in the words the master list ratified", () => {
    ...
    expect(said("saltTithes")).toEqual(["+3 gold per unique luxury"]);
    ...
    expect(said("ritesOfPassage")).toEqual([
      "completing a unit grants +10 faith",
    ]);
    ...
    expect(said("lordOfTheSea")).toEqual([
      "+1 production, +1 gold on every hex with a Fishing Boat",
    ]);
  });
});
...
describe("the governments’ deferred halves, built", () => {
  ...
  it("The Standing Army pays nothing at all, and never mints", () => {
    ...
    for (let i = 0; i < 4; i++) createUnit(g.state, 0, "warrior", 3, 3);
    const gross = unitUpkeepTotal(g.state, 0);
    playerById(g.state, 0)!.statecraft.doctrines.push(
      "theStandingArmy" as never,
    );
    bumpRevision(g.state);
    const rebate = explainUnitUpkeepRebate(g.state, 0);
  ...
  it("The Commonwealth raises a great person’s works and nothing else on the hex", () => {
    ...
    const shareLine = (): (typeof lines)[number] | undefined => {
      const found = explainTileYield(tile, yieldContextFor(g.state, 0)).filter(
        (line) => line.source.includes("Commonwealth"),
      );
      return found[0];
    };
    ...
    const works = lines.find(
      (line) => line.source === improvementDef("academy").name,
    )!;
  ...
  it("The Empire heals the army when the town it took held a wonder", () => {
    ...
    const withWonder = windfallPayout(g.state, 0, "capture", 0, 0, {
      capturedWonder: true,
    });
  ...
  it("The Commonwealth sells the recruitment, and no other law does", () => {
    ...
    expect(greatPersonPurchaseError(g.state, 0, "gold")).toContain(
      "law does not let",
    );
    govern(g.state, 0, "theCommonwealth");
    expect(greatPersonPurchaseError(g.state, 0, "gold")).toBeNull();
    // …and only out of the bank the law names.
    expect(greatPersonPurchaseError(g.state, 0, "faith")).toContain(
      "law does not let",
    );
  ...
  it("The Academy sells a draft of scholars for faith, and moves no renown", () => {
    ...
    expect(greatPersonPurchaseError(g.state, 0, "scholarDraft")).toContain(
      "law does not let",
    );
    ...
    expect(greatPersonPurchaseError(g.state, 0, "silver")).toContain(
      "purchaseGreatPersonOffer needs",
    );
    ...
    for (const id of offer.options)
      expect(greatPersonDef(id).family).toBe("scholar");
    expect(greatPersonBlocker(player)).not.toBeNull();
    expect(g.state.units.some((u) => u.person !== undefined)).toBe(false);
...
describe("the doctrines’ deferred halves, built", () => {
  it("The Encyclopaedia hurries a science building and no other", () => {
    ...
    expect(
      cardProduction(g.state, city, "building", undefined, "granary"),
    ).toHaveLength(0);
  ...
  it("The Academy trades culture for science, both at the empire stage", () => {
    ...
    const city = found(g.state, 0);
    expect(
      explainCardPercentYields(g.state, city).some(
        (l) => l.card === "theAcademyOfDeeds",
      ),
    ).toBe(false);
    playerById(g.state, 0)!.statecraft.doctrines.push(
      "theAcademyOfDeeds" as never,
    );
    bumpRevision(g.state);
    const lines = explainCardPercentYields(g.state, city).filter(
      (l) => l.card === "theAcademyOfDeeds",
    );
...
describe("the master-list cut of 2026-08-28, second pass", () => {
  it("authorityPositive — Bread and Circuses opens and closes with the writ", () => {
    ...
    expect(
      explainHappiness(g.state, 0).some((l) => l.source.includes("Bread")),
    ).toBe(false);
    ...
    city.population = 6;
    bumpRevision(g.state);
    const open = explainHappiness(g.state, 0).filter((l) =>
      l.source.includes("Bread"),
    );
    expect(open.length).toBeGreaterThan(0);
    // Two, not three, since the cards pass of 2026-09-05 trimmed the strongest
    // early Doctrine.
    expect(foldMeter(open)).toBe(
      2 * g.state.cities.filter((c) => c.ownerId === 0).length,
    );
    ...
    const gold = explainCardCityYields(g.state, city).find(
      (l) => l.card === "breadAndCircuses",
    )!;
  ...
  it("followingFaithPerTurn — Cuius Regio reads the faith of the towns that follow him", () => {
    ...
    const rate = (following: number): number =>
      foldCardYields(
        explainCardEmpireYields(g.state, 0, {
          followingFaithPerTurn: following,
        }),
      ).science;
    ...
    expect(
      foldCardYields(
        explainCardEmpireYields(g.state, 0, { followingFaithPerTurn: 60 }),
      ).faith,
    ).toBe(0);
  });
  ...
  it("capturedCityCost is a delta now, and two of them floor at one", () => {
    ...
    void city;
    const cost = (): number =>
      -explainAuthority(g.state, 0).find((l) => l.source.includes("captured"))!
        .value;
  ...
  it("Master of Maps and The Legion move strength in flat points, not percent", () => {
    ...
    const lines = (): { source: string; value: number }[] => {
      ...
      return preview.ok
        ? preview.bonuses.map((b) => ({ source: b.source, value: b.amount }))
        : [];
    };
...
describe("the Orders pass of 2026-08-29", () => {
  ...
  it("populationAtMost — Hearth Songs pays the villages and stops at the fifth citizen", () => {
    ...
    const paid = (): number =>
      explainCardCityYields(g.state, city).filter(
        (l) => l.card === "hearthSongs",
      ).length;
    ...
    city.population = 4;
    expect(paid()).toBe(1);
    expect(foldCardYields(explainCardCityYields(g.state, city)).culture).toBe(
      2,
    );
    city.population = 5;
    expect(paid()).toBe(0);
  ...
  it("adjacentImprovement — The Pilgrim’s Purse pays the town next door to the shrine", () => {
    ...
    const faith = (): number =>
      foldCardYields(explainCardCityYields(g.state, city)).faith;
    expect(faith()).toBe(0);
  ...
  it("duplicateLuxuries — Village Fairs counts kinds, not copies", () => {
    const g = game(803);
    const city = found(g.state, 0);
    const seams = ownedTiles(g.state, city).filter(
      (t) => t.col !== city.col || t.row !== city.row,
    );
    slot(g.state, 0, "villageFairs");
    const fairs = (): number =>
      foldMeter(
        explainHappiness(g.state, 0).filter((l) =>
          l.source.includes("Village Fairs"),
        ),
      );
  ...
  it("hillCityCost — Hill Forts prices a hill town a point cheaper, and says so in the preview", () => {
    ...
    const lineFor = (): number =>
      explainAuthority(g.state, 0).find((l) =>
        l.source.startsWith(second.name),
      )!.value;
    const plain = lineFor();
    ...
    if (site) {
      const preview = explainFoundingCost(g.state, 0, site).find(
        (l) => l.meter === "authority",
      )!;
  ...
  it("cityRule — Cistern Works waters every town, and no hex at all", () => {
    ...
    expect(cityScopeAdmits(g.state, city, { test: "notFreshwater" })).toBe(
      true,
    );
  ...
  it("Charter Towns founds with a Granary, and Homestead Charters still adds the citizen", () => {
    ...
    const unit =
      g.state.units.find((u) => u.ownerId === 0 && u.type === "settler") ??
      g.state.units.find((u) => u.ownerId === 0)!;
    const city = foundCityAt(
      g.state,
      0,
      getTileAt(g.state.map, unit.col, unit.row)!,
    )!;
  ...
  it("prints every new row in the words the user ratified", () => {
    ...
    expect(said("hearthSongs")).toEqual([
      "+2 culture in every city of 4 or fewer citizens",
    ]);
    expect(said("statuteLabour")).toEqual([
      "+1 production in every city per 4 citizens there",
    ]);
    ...
    expect(said("theMusterRoll")).toEqual([
    ...
    expect(said("thePilgrimsPurse")).toEqual([
      "+5 faith in every city beside a Holy Site",
    ]);
    expect(said("charterTowns")).toEqual([
      "new cities are founded with a Granary",
    ]);
...
describe("the balance pass of 2026-08-31", () => {
  it("unimproved + anyFeature — The Unbroken Land pays the standing woods and nothing else", () => {
    ...
    const paid = (): { food: number; production: number } => {
      ...
      return {
        ...
        production: lines.reduce(
          (sum, entry) => sum + (entry.production ?? 0),
          0,
        ),
      };
    };
  ...
  it("freeCitizens — The Scattered Hearths waives the first two of every town", () => {
    ...
    const demandLine = () =>
      explainHappiness(g.state, 0).find(
        (entry) =>
          entry.source.startsWith(city.name) &&
          entry.source.includes("citizens"),
      )!;
  ...
  it("reads every member this pass declared from at least one live card", () => {
    ...
    const rows = [...GOVERNMENT_IDS, ...DOCTRINE_IDS, ...ORDER_IDS].map((id) =>
      cardDef(id),
    );
  ...
  it("places every row this pass touched in the pool the doc puts it in", () => {
    ...
    expect(poolOrders("governmentIII").includes("theOldWays" as never)).toBe(
      true,
    );
    expect(
      poolOrders("governmentII").includes("foreignQuarters" as never),
    ).toBe(false);
  });
...
describe("the ratified cards of the Themes Build", () => {
  it("upkeepRebate — The Quartermasters take a coin off each soldier, never past free", () => {
    ...
    for (let i = 0; i < 3; i++)
      createUnit(g.state, 0, "warrior", seat.col, seat.row);
    ...
    // half has always had.
    expect(lines[0]!.gold).toBe(
      soldiers.reduce((sum, l) => sum + Math.min(1, l.gold), 0),
    );
    ...
    const ledger = explainEmpireGold(g.state, 0);
    expect(
      ledger.some((line) => line.source.includes("The Quartermasters")),
    ).toBe(true);
    expect(
      ledger.some((line) => line.source.startsWith("Unit maintenance")),
    ).toBe(true);
  });
  ...
  it("upkeepRebate free + unitStat outside — The Wintering Grounds keep an army in the field", () => {
    ...
    const abroad = explainUnitUpkeep(g.state, 0).filter(
      (line) => line.unitId === away.id,
    );
  ...
  it("queueHolds — The Wonder-Feasts feed the town with the scaffolding up", () => {
    ...
    const fed = (): number =>
      foldCardYields(
        explainCardCityYields(g.state, city).filter((l) =>
          l.source.includes("Wonder-Feasts"),
        ),
      ).food;
    expect(fed()).toBe(0);
    ...
    city.queue = [{ kind: "building", id: "stonehenge" }];
    expect(
      cardProduction(g.state, city, "wonder").find((l) =>
        l.source.includes("Wonder-Feasts"),
      )?.percent,
    ).toBe(20);
  });
  ...
  it("clearedCamps — The Last Hunt counts what the steppe no longer has", () => {
    ...
    const paid = explainCardEmpireYields(g.state, 0).find((line) =>
      line.source.includes("The Last Hunt"),
    );
    expect(paid?.culture).toBe(4);
    player.campsCleared = 4;
    expect(
      explainCardEmpireYields(g.state, 0).find((line) =>
        line.source.includes("The Last Hunt"),
      )?.culture,
    ).toBe(16);
  });
  ...
  it("internalTradeRoutes — The Provisioners pay for the grain that never leaves", () => {
    ...
    const second = foundCityAt(
      g.state,
      0,
      getTileAt(g.state.map, mine.col + 3, mine.row)!,
    );
  ...
  it("renown — The Laureate is a trickle now, and it joins the ledger it pays into", () => {
    ...
    found(g.state, 0);
    expect(
      cardRenownLines(g.state, 0).some((l) =>
        l.source.includes("The Laureate"),
      ),
    ).toBe(false);
    slot(g.state, 0, "theLaureate");
    const line = cardRenownLines(g.state, 0).find((l) =>
      l.source.includes("The Laureate"),
    );
  ...
  it("reads every shape this pass declared from at least one live card", () => {
    ...
    const noteScope = (scope: unknown): void => {
      ...
      for (const inner of (scope as { of?: unknown[] }).of ?? [])
        noteScope(inner);
    };
    for (const id of ORDER_IDS) {
      for (const effect of orderDef(id).effects) {
        noteScope((effect as { scope?: unknown }).scope);
        if (effect.kind === "pays" && effect.count !== undefined)
          counts.add(effect.count);
        if (effect.kind === "unitStat" && effect.where)
          wheres.add(effect.where);
        if (effect.kind === "upkeepRebate" && effect.where)
          wheres.add(effect.where);
        if (effect.kind === "productionBonus" && effect.class !== undefined)
          filtered = true;
      }
    }
  ...
  it("prints every new row in the words the sheets ratified", () => {
    ...
    expect(said("theCensusEternal")).toEqual(["+1 science per 2 citizens"]);
...
describe("the balance pass of 2026-09-02", () => {
  it("hasImprovement — Quarrymen\u2019s Guild asks for the quarry, not for the stone", () => {
    ...
    expect(
      explainCardCityYields(g.state, city).some(
        (l) => l.card === "quarrymensGuild",
      ),
    ).toBe(false);
  ...
  it("garrisoned — the scope still reads the pieces, and River Wardens no longer asks", () => {
    ...
    g.state.units = g.state.units.filter(
      (u) => u.col !== city.col || u.row !== city.row,
    );
    ...
    const admits = (): boolean =>
      cityScopeAdmits(g.state, city, { test: "garrisoned" });
    ...
    expect(
      scopedCardTileLines(g.state, city).some((line) =>
        line.source.includes("River Wardens"),
      ),
    ).toBe(false);
    expect(
      cardTileLines(g.state, 0).some((line) =>
        line.source.includes("River Wardens"),
      ),
    ).toBe(true);
  });
  ...
  it("yields — The Gilded Court reads what the hex has been reckoned to pay", () => {
    ...
    const paid = (tile: ReturnType<typeof getTileAt>): number =>
      explainTileYield(tile!, ctx).filter((l) =>
        l.source.includes("Gilded Court"),
      ).length;
    ...
    const line = explainTileYield(wet, ctx).find((l) =>
      l.source.includes("Gilded Court"),
    )!;
    ...
    expect(tileConditionHolds(wet, { test: "yields", yield: "gold" })).toBe(
      false,
    );
  });
  ...
  it("moves every number the doc moved", () => {
    ...
    expect(said("publicGranaries")).toEqual([
      "+15% of the stored food kept when a city grows",
    ]);
    expect(said("masterMasons")).toEqual([
      "completing a building grants +25 culture",
    ]);
    expect(said("theSaltRoad")).toEqual([
      "+1 gold on every hex carrying a strategic resource",
    ]);
    expect(said("spoilsOfTheWild")).toEqual([
      "clearing a barbarian camp pays +100%",
    ]);
...
describe("the playtest nerf batch of 2026-09-03", () => {
  it("retires The Greenwood Law from the Government II pool while keeping the row readable", () => {
    ...
    for (const pool of ORDER_POOLS)
      expect(poolOrders(pool)).not.toContain("theGreenwoodLaw");
    // Still a card: a save that holds it slotted still replays, and it still
    // pays what it says.
    const g = game();
    const city = found(g.state, 0);
  ...
  it("retires Athenaeum of the Road — the first Doctrine ever withdrawn", () => {
    ...
    for (const tier of GOVERNMENT_TIERS) {
      expect(poolDoctrines(tier), `tier ${tier}`).not.toContain(
        "athenaeumOfTheRoad",
      );
    }
    ...
    expect(
      describeCard("athenaeumOfTheRoad").map((clause) =>
        stripRefs(clause.text),
      ),
    ).toEqual(["a ruin you claim pays every option instead of one"]);
  });
  ...
  it("reads the anyFeature list from a live row, and prints it in the card’s own words", () => {
    ...
    ).toBe("forest or jungle hex");
    expect(
      tileConditionWords({
        test: "all",
        of: [
          { test: "unimproved" },
          { test: "anyFeature", features: ["forest", "jungle"] },
        ],
      }),
  ...
  it("answers anyFeature off the hex’s own feature, and nothing else", () => {
    ...
    tile.feature = "forest";
    expect(tileConditionHolds(tile, { test: "anyFeature", features: [] })).toBe(
      false,
    );
  });
});
...
describe("the Order draft", () => {
  ...
  function drafts(g: ReturnType<typeof game>, count = 40): OrderId[][] {
    ...
    for (let i = 0; i < count; i++)
      hands.push(drawOrderOffer(g.state, player).options);
    return hands;
  }
  ...
  it("deals the current government’s pool and nothing behind it", () => {
    for (const seed of [7, 11, 23, 41, 97]) {
      ...
      bumpRevision(g.state);
      expect(poolOfGovernment(player.statecraft.government)).toBe(
        "governmentII",
      );
      for (const hand of drafts(g)) {
        for (const id of hand)
          expect(orderDef(id).pool, `${seed} ${id}`).toBe("governmentII");
      }
    }
  });
  ...
  it("shows one of every slot type in a hand of three", () => {
    ...
    for (const pool of ORDER_POOLS) {
      for (const type of SLOT_TYPES) {
        expect(
          poolOrders(pool).some((id) => orderDef(id).slot === type),
          `${pool} ${type}`,
        ).toBe(true);
      }
    }
      ...
      for (const government of ["chiefdom", "republic"] as const) {
        ...
        for (const hand of drafts(g)) {
          ...
          for (const type of SLOT_TYPES) {
            expect(
              types.has(type),
              `${seed} ${government} ${type} in ${hand.join()}`,
            ).toBe(true);
          }
        }
      }
  ...
  it("deals a legal hand from a pool with no military left in it", () => {
    ...
    expect(livePool(sc).some((id) => orderDef(id).slot === "military")).toBe(
      false,
    );
  ...
  it("keeps the plain uniform draw for a hand narrower than the spread", () => {
    ...
    expect(
      [...seen].some((pair) => pair.split("+")[0] === pair.split("+")[1]),
    ).toBe(true);
  });
});
...
describe("the user's card pass of 2026-09-03", () => {
  ...
  it("yieldConversion — Thalassocracy mints a tenth of what a coastal town grows", () => {
    ...
    expect(
      cardYieldConversions(g.state, city, { ...flats, food: 9 })[0]!.production,
    ).toBe(0.9);
    expect(cardYieldConversions(g.state, city, { ...flats, food: 0 })).toEqual(
      [],
    );
    expect(
      cardYieldConversions(g.state, city, { ...flats, food: 10 })[0]!
        .production,
    ).toBe(1);
  ...
  it("atPopulation — First Fruits pays for the first citizen and for no other", () => {
    ...
    expect(
      windfallPayout(g.state, 0, "growth", 0, 0, { population: 2 }).grants,
    ).toEqual([
      {
        card: "firstFruitsOffering",
        source: "Order · First Fruits",
        yield: "faith",
        amount: 10,
      },
    ]);
    expect(
      windfallPayout(g.state, 0, "growth", 0, 0, { population: 3 }).grants,
    ).toEqual([]);
    // An occasion that carries no population at all never satisfies it.
    expect(windfallPayout(g.state, 0, "growth").grants).toEqual([]);
    expect(
      windfallPayout(g.state, 0, "chop", 0, 0, { population: 2 }).grants,
    ).toEqual([]);
  ...
  it("The Sacred Path pays the canopy in two voices", () => {
    ...
    const city = found(g.state, 0);
    const player = playerById(g.state, 0)!;
    const tile = ownedTiles(g.state, city).find(
      (t) => t.col !== city.col || t.row !== city.row,
    )!;
    const ctx = () => yieldContextFor(g.state, 0);
    ...
    expect(open.some((line) => line.source.includes("Sacred Path"))).toBe(
      false,
    );
  });
  ...
  it("takes the three withdrawn rows out of every pool, and keeps them readable", () => {
    ...
    expect(
      poolOrders("chiefdom").includes("firstFruitsOffering" as never),
    ).toBe(false);
  ...
  it("reads the shape this pass declared from at least one live card", () => {
    ...
    expect(
      [...GOVERNMENT_IDS, ...DOCTRINE_IDS, ...ORDER_IDS].some((id) =>
        cardDef(id).effects.some(
          (effect) => effect.kind === "pays" && effect.basis === "share",
        ),
      ),
    ).toBe(true);
    ...
    expect(riders.some((effect) => effect.atPopulation !== undefined)).toBe(
      true,
    );
  });
  ...
  it("prints every changed and new row in the words the user ratified", () => {
    ...
    expect(said("firstFruitsOffering")).toEqual([
      "a city growing grants +10 faith",
    ]);
  });
});
...
describe("the card-shapes pass of 2026-09-04", () => {
  ...
  function flats(
    over: Partial<Record<CityYieldKey, number>> = {},
  ...
  function empireLine(
    ...
    const lines = explainCardEmpireYields(state, playerId).filter((entry) =>
      entry.source.includes(name),
    );
  ...
  it("slottedOrdersOfSlot — First Rites reads wildcards, by the card and not the chair", () => {
    ...
    expect(slottedOrdersOfFlavour(g.state, 0)).toBe(
      filledOrderSlots(g.state, 0),
    );
  });
  ...
  it("capital — The Guild Charter's hammers land in one town, and only there", () => {
    ...
    expect(
      explainCardEmpireYields(g.state, 0).some((line) => line.production !== 0),
    ).toBe(false);
  });
  ...
  it("atWar — The Arsenal Law opens the yards on a declaration and closes them on a peace", () => {
    ...
    const paid = (): number => {
      const lines = cardYieldConversions(
        g.state,
        city,
        flats({ production: 100 }),
      );
      return (
        lines.find((line) => line.source.includes("The Arsenal Law"))?.gold ?? 0
      );
    };
  ...
  it("yieldConversion — the four new pairs, each with its own gate", () => {
    ...
    const paid = (
      ...
      cardYieldConversions(g.state, city, flats(over)).find((line) =>
        line.source.includes(name),
      )?.[voice] ?? 0;
  ...
  it("reads every member this pass declared from at least one live card", () => {
    ...
    const walk = (effects: readonly CardEffect[]): void => {
      for (const effect of effects) {
        ...
        if (effect.kind === "combatLine" && effect.scaled)
          scales.add(effect.scaled.count);
...
describe("the synergy-density pass of 2026-09-05", () => {
  ...
  it("Harbour Dues — the Tide’s conversion, and it stops at the shore", () => {
    ...
    const shore = isCoastal(
      g.state.map,
      getTileAt(g.state.map, city.col, city.row)!,
    );
  ...
  it("Scholars’ Stipend — a Library then a University, both behind the fifth citizen", () => {
    ...
    const paid = (): number =>
      paidTo(g.state, city, "Scholars' Stipend", "science");
    city.population = 5;
    expect(paid()).toBe(0);
    city.buildings.push("library");
    bumpRevision(g.state);
  ...
  it("Ore Tithes — the seam’s hammer, and the Forge Levy’s reader beside it", () => {
    ...
    const hammers = (): number =>
      paidTo(g.state, capital, "Ore Tithes", "production");
    ...
    expect((orderDef("oreTithes").effects[0] as { where?: string }).where).toBe(
      "hex",
    );
  ...
  it("The Banner-Call — both halves open on a war and close on a peace", () => {
    ...
    const song = (): number =>
      windfallPayout(g.state, 0, "kill")
        .grants.filter((grant) => grant.yield === "culture")
    ...
    // whole of that rule.
    expect(
      cardProduction(g.state, city, "building", undefined, "granary").some(
        (line) => line.source.includes("The Banner-Call"),
      ),
    ).toBe(false);
    closeWar(g.state, 0, 1);
    bumpRevision(g.state);
  ...
  it("The Far Charts — the Wayfarers’ payoff, and the half that is not built", () => {
    ...
    const seen = g.state.visibility[0]!.reduce(
      (sum: number, bit: number) => sum + (bit > 0 ? 1 : 0),
      0,
    );
  ...
  it("The Wolf-Standard — deferred whole, and out of every pool", () => {
    ...
    expect(
      poolOrders("governmentII").includes("theWolfStandard" as never),
    ).toBe(false);
  });
...
describe("the cards pass of 2026-09-05", () => {
  it("opens a shelf of its own at every rung of the ladder", () => {
    ...
    for (const id of governmentsAtTier(29))
      expect(poolOfGovernment(id), id).toBe("governmentIV");
    for (const id of governmentsAtTier(45))
      expect(poolOfGovernment(id), id).toBe("governmentV");
    ...
    expect(poolOrders("governmentIV").length).toBeGreaterThanOrEqual(
      RULES.offers.order,
    );
    expect(poolOrders("governmentV").length).toBeGreaterThanOrEqual(
      RULES.offers.order,
    );
  });
  ...
  it("takes the eight cuts out of every pool and leaves them readable", () => {
    ...
    for (const id of cut) {
      ...
      for (const pool of ORDER_POOLS)
        expect(poolOrders(pool).includes(id), id).toBe(false);
    }
  });
  ...
  it("The Founding Oath pays the capital, in every voice, and stops at three", () => {
    ...
    const second = foundCityAt(
      ...
      getTileAt(
        g.state.map,
        (capital.col + 5) % g.state.map.width,
        capital.row,
      )!,
    )!;
  ...
  it("prints the late rows, and says out loud which half was struck", () => {
    const said = (id: string): string[] =>
      describeCard(id as never).map((c) => stripRefs(c.text));
    expect(said("theLongRoads")).toEqual([
      "+1 gold per road hex you have laid",
    ]);
  ...
  it("leaves the late Doctrines paying exactly what they print", () => {
    ...
    for (const id of [
      ...
      expect(
        describeCard(id).every((c) => !c.text.includes("not built yet")),
        id,
      ).toBe(true);
    }
...
describe("the Æra III fork of 2026-09-05", () => {
  ...
  it("Imperium — every city reads the military bench", () => {
    ...
    expect(
      cardUnitStat(
        g.state,
        createUnit(g.state, 0, "warrior", 3, 3),
        "movement",
      ),
    ).toBe(1);
  });
  ...
  it("Merchant League — the economic bench, the routes, and one more of them", () => {
    ...
    const coin = (): number =>
      foldCardYields(
        explainCardEmpireYields(g.state, 0).filter(
          (line) => line.card === "merchantLeague",
        ),
      ).gold;
    expect(coin()).toBe(0);
    slot(g.state, 0, "weightsAndMeasures");
    ...
    expect(slots.some((line) => line.source.includes("Merchant League"))).toBe(
      true,
    );
    expect(routeSlots(g.state, 0)).toBe(
      slots.reduce((sum, line) => sum + line.slots, 0),
    );
  });
  ...
  it("The Iron Price — twenty a kill, and a pillage worth twice as much", () => {
    ...
    expect(windfallPayout(g.state, 0, "kill").grants).toEqual([
      {
        card: "ironPrice",
        source: "Doctrine · The Iron Price",
        yield: "culture",
        amount: 20,
      },
    ]);
  ...
  it("Master of Maps — the science riders are off the row", () => {
    ...
    bumpRevision(g.state);
    const beakers = (
      occasion: "veinFound" | "discovery" | "prospect",
    ): number =>
      windfallPayout(g.state, 0, occasion)
        .grants.filter((grant) => grant.yield === "science")
        .reduce((sum, grant) => sum + grant.amount, 0);
    expect(beakers("veinFound")).toBe(0);
    expect(beakers("discovery")).toBe(0);
    expect(beakers("prospect")).toBe(0);
    expect(
      doctrineDef("masterOfMaps").effects.some(
        (effect) => effect.kind === "windfallRider",
      ),
    ).toBe(false);
  });
  ...
  it("veinFound — the strike is fired from the survey, and only on a strike", () => {
    ...
    expect(dry.lines.some((line) => line.card === "theDeepDelving")).toBe(
      false,
    );
    ...
    expect(report.lines.some((line) => line.card === "theDeepDelving")).toBe(
      true,
    );
  });
  ...
  it("The Pilgrim Ways — the faith fork’s permanent pick", () => {
    ...
    const theirs = found(g.state, 1);
    playerById(g.state, 0)!.statecraft.doctrines.push(
      "thePilgrimWays" as never,
    );
    bumpRevision(g.state);
    const paid = (key: CityYieldKey, rates = {}): number =>
      foldCardYields(
        explainCardEmpireYields(g.state, 0, rates).filter(
          (line) => line.card === "thePilgrimWays",
        ),
      )[key];
  ...
  it("The Natural Philosophers — the capital’s shelves, and a technology’s song", () => {
    ...
    const capital = found(g.state, 0);
    playerById(g.state, 0)!.statecraft.doctrines.push(
      "theNaturalPhilosophers" as never,
    );
    bumpRevision(g.state);
  ...
  it("The Deep Delving — the workings, and what a surfaced seam is worth", () => {
    ...
    expect(windfallPayout(g.state, 0, "veinFound").grants).toEqual([
      {
        card: "theDeepDelving",
        source: "Doctrine · The Deep Delving",
        yield: "gold",
        amount: 40,
      },
    ]);
...
describe("a charter carries the description of the building it opens", () => {
  ...
  it("says every live clause the building’s own describer says", () => {
    for (const { card, building } of OPENERS) {
      ...
      const row = describeBuildingRow(building).filter(
        (entry) => entry.deferred !== true,
      );
      expect(row.length, building).toBeGreaterThan(0);
      for (const entry of row)
        expect(said, `${card} → ${building}`).toContain(stripRefs(entry.text));
      ...
      for (const entry of describeBuildingRow(building)) {
        ...
        expect(said, `${card} → ${building}`).not.toContain(
          stripRefs(entry.text),
        );
      }
  ...
  it("composes at print time and leaves the ratified text alone", () => {
    ...
    const said = describeCard("ritesCharter").map((entry) =>
      stripRefs(entry.text),
    );
    expect(said).toEqual([
      "unlocks the Chapel — +1 faith; a rite performed in this city pays +5 culture",
    ]);
  });
  ...
  it("reads the half of a building row that no card effect models", () => {
    ...
    const words = (id: BuildingId): string =>
      describeBuildingRow(id)
        ...
        .join(" · ");
    expect(words("chapel")).toContain(
      "a rite performed in this city pays +5 culture",
    );
...
describe("the remembered walk", () => {
  ...
  it("moves with an accepted command and stands still on a refused one", () => {
    ...
    expect(
      dispatch(g, { type: "fortify", playerId: 0, unitId: 99_999 }).ok,
    ).toBe(false);
    expect(liveEffects(g.state, 0)).toBe(held);
    expect(
      dispatch(g, { type: "chooseResearch", playerId: 0, techId: "mining" }).ok,
    ).toBe(true);
  ...
  it("drops a rite the turn it runs out", () => {
    ...
    seat.timed = [
      {
        card: "riteOfPlenty",
        effect: { kind: "happiness", amount: 2 },
        expiresTurn: state.turn + 1,
      },
    ];
    bumpRevision(state);
    expect(lawOf(state, 0).some((line) => line.includes("turns left"))).toBe(
      true,
    );
  ...
  it("drops a legacy the turn it is revoked", () => {
    ...
    bumpRevision(state);
    expect(lawOf(state, 0).some((line) => line.includes("Imhotep"))).toBe(
      false,
    );
  });
  ...
  it("follows a wonder to the empire that takes the town it stands in", () => {
    ...
    expect(lawOf(state, 1).some((line) => line.startsWith("Wonder"))).toBe(
      false,
    );
    ...
    city.ownerId = 1;
    bumpRevision(state);
    expect(lawOf(state, 0).some((line) => line.startsWith("Wonder"))).toBe(
      false,
    );
    expect(lawOf(state, 1).some((line) => line.startsWith("Wonder"))).toBe(
      true,
...
describe("the memo’s key", () => {
   ...
   */
  const SIM_SOURCE = import.meta.glob(
  ...
  function bodyOf(name: string): string {
    ...
    expect(`${name} found`).toBe(
      from === -1 ? `${name} missing` : `${name} found`,
    );
...
describe("the engine shapes", () => {
  ...
  function withCards(
    ...
    try {
      ...
      for (const [id, effects] of held) {
        (orderDef(id) as { effects: CardEffect[] }).effects =
  ...
  it("takes the multiplicative variant off what the other card printed", () => {
    withCards(
      [
        ...
        [
          "theChoir",
          [{ kind: "cardYieldAmplifier", yield: "faith", percent: 50 }],
        ],
      ],
      () => {
        const g = game();
  ...
  it("keeps a scoped amplifier out of the empire fold and off the ground", () => {
    withCards(
      ...
      () => {
        ...
        expect(
          explainCardEmpireYields(g.state, 0).some(
            (line) => line.card === "theChoir",
          ),
        ).toBe(false);
        expect(
          cardTileLines(g.state, 0).some((line) =>
            line.source.includes("Choir"),
          ),
        ).toBe(false);
        void city;
      },
  ...
  it("does reach the capital’s own ledger, and says which town it landed in", () => {
    withCards(
      ...
      () => {
        ...
        seat(g.state, 0, 0, "waysideShrines");
        seat(g.state, 0, 1, "theChoir");
        const paid = explainCardCityYields(g.state, city).find(
          (line) => line.card === "theChoir",
        )!;
  ...
  it("reads a belief’s faith on the hex — the asking lines land after the paying ones", () => {
    ...
    expect(
      explainTileYield(tile, yieldContextFor(g.state, 0)).some(
        (l) => l.faith > 0,
      ),
    ).toBe(false);
    ...
    expect(
      lines.some(
        (l) =>
          l.source.includes(orderDef("theSacredGround").name) && l.faith === 1,
      ),
    ).toBe(true);
    // The asking line is the one the fold holds back; the paying line is not.
    expect(tileConditionReadsFold({ test: "yields", yield: "faith" })).toBe(
      true,
    );
    expect(tileConditionReadsFold({ test: "terrain", terrain: "desert" })).toBe(
      false,
    );
    expect(
      tileConditionReadsFold({
        test: "all",
        of: [
          { test: "terrain", terrain: "desert" },
          { test: "yields", yield: "faith" },
        ],
      }),
    ).toBe(true);
  });
  ...
  it("stamps an absolute turn, fires on it, and re-stamps — nothing ticks", () => {
    withCards(
      [
        [
          "waysideShrines",
          [{ kind: "periodic", everyTurns: 5, pays: "gold", amount: 10 }],
        ],
      ],
      () => {
        const g = game();
      ...
      () => {
        ...
        expect(player.faithPool - before).toBe(
          3 +
            Math.max(
              0,
              Math.floor(foldEmpireRates(g.state, 0).productionPerTurn ?? 0),
            ),
        );
      },
    );
  ...
  it("composes a boon’s riders into one printed figure before it is banked", () => {
    withCards(
      [
        [
          "waysideShrines",
          [{ kind: "periodic", everyTurns: 2, pays: "gold", amount: 10 }],
        ],
        [
          "theChoir",
          [
  ...
  it("adds a share of one town’s own trickle, naming no family", () => {
    ...
    const raised = explainCityRenown(city, [
      { source: "Heroic Epic", percent: 50 },
    ]);
    ...
    expect(explainCityRenown(city, [{ source: "X", percent: 0 }])).toHaveLength(
      2,
    );
  });
  ...
  it("puts a card’s yields on the caravan, where a doubler can find them", () => {
    withCards(
      [
        [
          "waysideShrines",
          [{ kind: "pays", where: "route", food: 1, production: 2 }],
        ],
      ],
      () => {
        const g = game();
        ...
        const before = foldRouteYield(
          explainRouteYieldBetween(g.state, from, to),
        );
        seat(g.state, 0, 0, "waysideShrines");
        const after = foldRouteYield(
          explainRouteYieldBetween(g.state, from, to),
        );
  ...
  it("narrows a route line to the town the caravan left", () => {
    withCards(
          ...
          [
            {
              ...
              where: "route",
              gold: 4,
              origin: { test: "hasBuilding", building: "market" },
            },
          ],
        ],
      ],
      () => {
        ...
        seat(g.state, 0, 0, "waysideShrines");
        const shut = foldRouteYield(
          explainRouteYieldBetween(g.state, from, to),
        );
        from.buildings.push("market");
        bumpRevision(g.state);
        const open = foldRouteYield(
          explainRouteYieldBetween(g.state, from, to),
        );
  ...
  it("registers exactly which live rows use each shape batch A declared", () => {
    ...
    const walk = (
      ...
      for (const effect of effects ?? []) {
        ...
        if (
          effect.kind === "periodic" &&
          effect.count !== undefined &&
          NEW_COUNTS.has(effect.count)
        ) {
          seen.push(`${where} · ${effect.count}`);
        }
...
describe("the order pass of 2026-09-06", () => {
  ...
  it("folds every row it touched, in a chair, without moving the state", () => {
    ...
    for (const id of TOUCHED) {
      ...
      slot(g.state, 0, id);
      expect(
        liveEffects(g.state, 0).some((e) => e.card === id),
        id,
      ).toBe(orderDef(id).effects.length > 0);
      const quote = explainCity(g.state, city);
      for (const key of CITY_YIELD_KEYS)
        expect(Number.isFinite(quote.flats[key]), id).toBe(true);
      const paid = foldCity(g.state, city);
      for (const key of CITY_YIELD_KEYS)
        expect(Number.isFinite(paid[key]), id).toBe(true);
      ...
      const board = (print: string): string =>
        print.replace(/"revision":\d+/, '"revision":0');
  ...
  it("says every touched row in words, with nothing left silent", () => {
    for (const id of TOUCHED) {
      ...
      for (const clause of clauses) {
        ...
        expect(
          stripRefs(clause.text).includes("[["),
          `${id}: ${clause.text}`,
        ).toBe(false);
      }
      expect(def.text, id).toBeTruthy();
  ...
  it("pays the four Æra V bead Orders on one shape, each on its own deed", () => {
    ...
    for (const [id, occasion] of Object.entries(deeds)) {
      ...
      // The bead is a **repeatable grant** row, which is the whole of what let
      // one empire mint it more than once (`BeadGrantDef.repeatable`) — and it
      // is retired with the card that named it.
      expect(beadGrantDef(effect.bead).repeatable, id).toBe(true);
      expect(beadGrantDef(effect.bead).retired, id).toBe(true);
      minted.add(effect.bead);
      // Nothing is left struck through on a row that now does what it says.
      expect(def.deferred, id).toBeUndefined();
      // **Retired by batch Q1** (`docs/flags.md` (nnn), ruling 2): the four were
      // waiting on deeds and the deeds left the game, so the cards carry the
      // mark, the note is the standing retirement sentence, and the rows are out
      // of the bag the draft deals from. The *shape* is untouched — the row
      // still says what it would pay, which is what makes this test still worth
      // reading — and the grant it names is withdrawn beside it.
      expect(def.retired, id).toBe(true);
      expect(def.note, id).toContain("no longer offered");
      expect(def.pool, id).toBe("governmentV");
      expect(poolOrders("governmentV").includes(id as OrderId), id).toBe(false);
    }
    ...
    const capped = (id: OrderId): (number | undefined)[] =>
      orderDef(id).effects.flatMap((effect) =>
        effect.kind === "pays" && effect.count === "slottedOrdersOfSlot"
          ? [effect.max]
          : effect.kind === "combatLine" &&
              effect.scaled?.count === "slottedOrdersOfSlot"
            ? [effect.scaled.max]
            : [],
      );
...
describe("the cadence and the chairs, as ruled on the third pass", () => {
  ...
  it("takes a quarter off the chairs at tiers 18, 29 and 45, and leaves the early ones alone", () => {
    ...
    const chairs = (id: GovernmentId): number =>
      spread(id).reduce((sum, n) => sum + n, 0);
...
describe("an Order dealt only from its age", () => {
  ...
  it("is the four bead Orders of pool V, gated on the last age and now retired", () => {
    ...
    for (const id of gated) {
      ...
      // Batch Q1: they were waiting on deeds. The gate stays on the row.
      expect(orderDef(id).retired, id).toBe(true);
    }
  });

  it("is off the shelf outright now the row is retired", () => {
    // **Re-aimed by batch Q1.** The gate is untouched and unreachable: the four
    // rows that carried `fromAge` are the four bead Orders, and they are retired
    // with the deeds they counted, so they leave `livePool` a rung above the
    // age question. What is pinned is that both readings agree — a retired row
    // is off the shelf and out of the bag at every age.
    const g = game();
    const sc = playerById(g.state, 0)!.statecraft;
    sc.government = "theEmpire";
    bumpRevision(g.state);
    for (const id of gated) {
      expect(livePool(sc), id).not.toContain(id);
      expect(drawablePool(sc, 3), id).not.toContain(id);
      expect(drawablePool(sc, 4), id).not.toContain(id);
    }
    // And nothing else in the pool minds the age, which is what leaves the gate
    // a rule with no live row asking for it.
    expect(drawablePool(sc, 1)).toEqual(livePool(sc));
  });
  ...
  it("never deals one to an empire still in an earlier age", () => {
    for (const seed of [1, 2, 3, 4, 5, 6, 7, 8]) {
      ...
      const offer = drawOrderOffer(g.state, player);
      for (const id of offer.options)
        expect(gated, `seed ${seed}`).not.toContain(id);
    }
  });

  it("keeps the chair of a row already held, whatever the age or the mark says", () => {
    const g = game();
    const player = playerById(g.state, 0)!;
    player.statecraft.government = "theEmpire";
    bumpRevision(g.state);
    slot(g.state, 0, gated[0]!);
    expect(player.statecraft.slots.map((s) => s?.card ?? null)).toContain(
      gated[0],
    );
...
describe("a building’s share counts what the law put on it", () => {
  it("takes the Synod’s half over the temple’s row and The Choir’s culture on it", () => {
    ...
    const synod = explainCardBuildingYields(g.state, city).find(
      (line) => line.card === "theSynod",
    )!;
    ...
    const choir = explainCardCityYields(g.state, city).find(
      (line) => line.card === "theChoir",
    )!;
...
describe("the deferred rows of batch E4a", () => {
  ...
  it("Triumphs banks renown for a town taken, through the one seam that pays it", () => {
    ...
    expect(payout.renown).toEqual([
      {
        card: "triumphs",
        source: expect.stringContaining("Triumphs"),
        amount: 25,
      },
    ]);
    payWindfallGrants(g.state, player, payout);
...
describe("terrainBeside — the third ring in the family", () => {
  it("admits the centre and the six around it, and nothing further out", () => {
    ...
    expect(cityScopeAdmits(g.state, city, scope)).toBe(true);
    expect(
      cityScopeAdmits(g.state, city, { test: "onTerrain", terrain: "desert" }),
    ).toBe(false);
...
describe("the Orders balance pass of 2026-09-08", () => {
  ...
  it("The Muses' Call — renown banked behind the closed door is answered at once", () => {
    ...
    settleRenownWindfall(g.state, player, [
      { family: null, amount: renownThreshold(player) },
    ]);
    ...
    bumpRevision(g.state);
    expect(
      settleRenownWindfall(g.state, player, [{ family: null, amount: 0 }]),
    ).not.toBeNull();
  ...
  it("The Muses' Call — prints its two standing clauses", () => {
    ...
    expect(doctrineDef("musesCall").onAdopt).toEqual([
      { grant: "greatPerson" },
    ]);
  ...
  it("Boatwrights — a hammer in every coastal town and nowhere else", () => {
    ...
    expect(
      explainCardCityYields(inland.state, dry).filter(
        (line) => line.card === "boatwrights",
      ),
    ).toEqual([]);
  });
  ...
  it("the two new Orders print what their rows say, in the Tide", () => {
    expect(said("boatwrights")).toEqual([
  ...
  it("The Great Warring Tribes — the hammers stand without the authority gate", () => {
    ...
    expect(
      doctrineDef("greatWarringTribes").effects.some(
        (effect) => effect.kind === "conditionRule",
      ),
    ).toBe(false);
  });
  ...
  it("Manifest of the Steppe — the happiness bill is dropped", () => {
    ...
    const before = foldMeter(explainHappiness(g.state, 0));
    playerById(g.state, 0)!.statecraft.doctrines.push(
      "manifestOfTheSteppe" as never,
    );
    bumpRevision(g.state);
    expect(foldMeter(explainHappiness(g.state, 0))).toBe(before);
    expect(
      doctrineDef("manifestOfTheSteppe").effects.some(
        (effect) => effect.kind === "meterRule",
      ),
    ).toBe(false);
  });
  ...
  it("Pax Imperia — the culture is a share of a great city now", () => {
    ...
    const share = (target: City): number =>
      explainCardPercentYields(g.state, target)
        .filter(
          (line) => line.card === "paxImperia" && line.yield === "culture",
        )
        .reduce((sum, line) => sum + line.percent, 0);
    city.population = 4;
    refreshCityDerived(g.state, city);
  ...
  it("Divine Inspiration leaves the bag and keeps its row", () => {
    ...
    expect(poolDoctrines(10).includes("divineInspiration" as never)).toBe(
      false,
    );
...
describe("the Governments marks of 2026-09-08", () => {
  ...
  it("War Chief — three points of writ, and the kill riders still stand", () => {
    ...
    bumpRevision(g.state);
    expect(
      windfallPayout(g.state, 0, "kill").grants.map((grant) => [
        grant.yield,
        grant.amount,
      ]),
    ).toEqual([
  ...
  it("Tyranny — five points of writ and two of strength", () => {
    ...
    expect(
      governmentDef("tyranny").effects.some(
        (effect) =>
          effect.kind === "rulePercent" && effect.rule === "unitUpkeep",
      ),
    ).toBe(true);
  });
  ...
  it("Theocracy — a fifth of the capital’s faith, in both voices", () => {
    ...
    expect(
      conversions.map((effect) =>
        effect.kind === "pays" ? effect.per : undefined,
      ),
    ).toEqual([5, 5]);
  });
...
describe("the deferred rows of batch E4b", () => {
  ...
  it("Admiralty lands the men free and blesses them for three turns", () => {
    ...
    for (const tech of ["sailing", "wayfinding"] as const) {
      if (!owner.techsResearched.includes(tech))
        owner.techsResearched.push(tech);
    }
    bumpRevision(g.state);
    ...
    g.state.turn += 3;
    expect(
      cardCombatLines(g.state, situation).filter(
        (line) => line.card === "admiralty",
      ),
    ).toEqual([]);
  });
  ...
  it("the Stable wants a pasture or a camp, pays both, and hurries the horses", () => {
    ...
    bumpRevision(g.state);
    expect(buildError(g.state, 0, "building", "stable", city)).toContain(
      "pasture",
    );
    ...
    const without = foldTileLines(
      explainTileYield(near, cityContext(g.state, city)),
    ).production;
  ...
  it("the Printing House is paid by the roads that end at its presses", () => {
    ...
    bumpRevision(g.state);
    expect(
      foldRouteYield(explainRouteYieldBetween(g.state, from, to)).science,
    ).toBe(plain.science);
