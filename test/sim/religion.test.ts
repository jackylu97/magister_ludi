...
/**
 * Dries the fixture's field. Seed 7's duel lays coast under several of the
 * hexes these fights are staged on, and since N1 (schema 107) a warrior cannot
 * strike at the water — so a test that is about a *line* on the ledger, and not
 * about where the blow lands, stands its pieces on grassland first. Terrain is
 * not a dice roll, so nothing downstream of the fixture moves.
 */
function dryField(state: GameState): void {
...
describe("the religion table", () => {
  ...
  it("gives every belief a description the screen can print", () => {
    for (const id of BELIEF_IDS) {
      ...
      for (const clause of clauses)
        expect(clause.text.length, id).toBeGreaterThan(0);
    }
  });
  ...
  it("teaches each live rite through its own technology, as an ability", () => {
    ...
    for (const id of LIVE_RITE_IDS) {
      ...
      expect(techDef(tech).unlocks.abilities ?? [], id).toContain(
        riteAbility(id),
      );
    }
...
describe("the augur is withdrawn, and the row is kept for replay", () => {
  it("is refused by the production queue and by its own bank, byte-identically", () => {
    ...
    expect(buildError(g.state, 0, "unit", "augur")).toBe(
      "Augurs are no longer called",
    );
...
describe("consecrate", () => {
  ...
  it("is refused always — the faith ladder deals the gods now", () => {
    ...
    expect(consecrateError(g.state, 1, augur.id)).toContain(
      "does not belong to player 1",
    );
    const worker = createUnit(g.state, 0, "worker", 5, 5);
    expect(consecrateError(g.state, 0, worker.id)).toBe(
      "A Worker cannot consecrate",
    );
  });
  ...
  it("refuses an index off the end of an offer, byte-identically", () => {
    ...
    player.pantheon.pending = {
      options: beliefPool(g.state, player).slice(0, 3),
    };
    const before = snapshotState(g.state);
    expect(
      applyCommand(g.state, {
...
describe("a belief is an effect source, not a second evaluator", () => {
  ...
  it("scopes a tile line by terrain (Desert Fathers)", () => {
    ...
    expect(
      explainTileYield(desert, ctx).some((line) =>
        line.source.startsWith("Belief ·"),
      ),
    ).toBe(true);
  });
  ...
  it("merges two lines from the same source into one entry (Winter Mother, tundra forest)", () => {
    ...
    const wooded = {
      ...base,
      terrain: "tundra" as const,
      feature: "forest" as const,
    };
    const before = foldTileLines(explainTileYield(wooded, ctx));
    const list = explainTileYield(wooded, ctx);
    const winterMotherLines = list.filter(
      (entry) => entry.source === "Belief · Winter Mother",
    );
  ...
  it("keeps two different sources on one tile as two entries (Winter Mother + Spirits of the Wood)", () => {
    ...
    const list = explainTileYield(wooded, ctx);
    const cardLines = list.filter((entry) =>
      entry.source.startsWith("Belief ·"),
    );
    expect(cardLines.map((entry) => entry.source).sort()).toEqual([
      "Belief · Spirits of the Wood",
      "Belief · Winter Mother",
    ]);
    const winter = cardLines.find(
      (entry) => entry.source === "Belief · Winter Mother",
    )!;
    expect(winter).toMatchObject({ food: 1, faith: 1 });
    const spirits = cardLines.find(
      (entry) => entry.source === "Belief · Spirits of the Wood",
    )!;
  ...
  it("narrows a tile line by resource kind and the voice it pays (Goddess of the Harvest)", () => {
    ...
    const foodOf = (tile: typeof base): number =>
      foldTileLines(explainTileYield(tile, ctx)).food;
    ...
    expect(foodOf(bare)).toBe(
      foldTileLines(explainTileYield(bare, undefined)).food,
    );
  });
  ...
  it("stands the calendar’s free unit in the capital, off the offer’s own hex", () => {
    ...
    const escort = DISCOVERY_IDS.find(
      (id) => discoveryDef(id).effect.kind === "unit",
    )!;
    ...
    expect(
      wrappedDistance(g.state.map, tileHex(stood), tileHex(home)),
...
describe("rites", () => {
  ...
  it("need the door — the Chapel, asked of the marker and never of a name", () => {
    ...
    expect(
      (buildingDef("chapel") as { ritesDoor?: boolean }).ritesDoor,
    ).toBeUndefined();
  });
  ...
  it("refuse everything they should, byte-identically", () => {
    ...
    expect(riteError(g.state, 0, other.id, "omenReading")).toContain(
      "does not belong",
    );
    expect(riteError(g.state, 0, city.id, "nonsense")).toContain(
      "no rite called",
    );
    ...
    expect(riteError(g.state, 0, 9999, "omenReading")).toContain(
      "No city with id",
    );
    for (const rite of [
      "omenReading",
  ...
  it("food — every hex the town works that feeds it feeds it one more", () => {
    ...
    expect(paid.food).toBeGreaterThan(
      foldTileLines(explainTileYield(tile)).food,
    );
  });
  ...
  it("gold — a worked hex with a seam in it pays a coin more", () => {
    ...
    const tile = g.state.map.tiles.find(
      (t) =>
        t.resource !== undefined && tileIndex(g.state.map, t.col, t.row) >= 0,
    );
    ...
    if (tile) {
      ...
      const without = foldTileLines(
        explainTileYield(tile, cityContext(g.state, city)),
      ).gold;
  ...
  it("science — every building standing in the town adds a beaker", () => {
    ...
    expect(explainCity(g.state, city).flats.science).toBe(
      before + city.buildings.length,
    );
  });
  ...
  it("stamps an absolute expiry, and the report carries the turns it runs", () => {
    ...
    expect(
      city.timed!.every((entry) => entry.expiresTurn === done.expiresTurn),
    ).toBe(true);
  });
});
...
describe("a prophet says a rite over every town", () => {
  function ready() {
    ...
    const second = foundCityAt(
      g.state,
      0,
      getTileAt(g.state.map, first.col + 3, first.row)!,
    );
    const player = playerById(g.state, 0)!;
    player.faithPool = 1000;
  ...
  it("takes over from whatever a town was keeping, so it is still one at a time", () => {
    ...
    expect(cityRite(g.state, first)).toBe("omenReading");
    const live = first.timed!.filter(
      (entry) => entry.card === "riteOfTheHarvest",
    );
...
describe("the apostle", () => {
  ...
  it("proclaims at half a prophet’s weight, over six hexes", () => {
    ...
    expect(proclaimPreview(g.state, prophet.id)!.lump).toBe(
      RULES.religion.bombLump,
    );
    expect(proclaimPreview(g.state, prophet.id)!.range).toBe(
      RULES.religion.bombRange,
    );
  });
  ...
  it("lays on hands: mends every friendly piece on its hex and the six touching it", () => {
    ...
    expect(
      done.units.some((row) => row.unitId === beside.id && row.healed === 25),
    ).toBe(true);
    // One charge of two.
  ...
  it("leaves a relic in a cathedral town, one per cathedral, paying faith a turn", () => {
    ...
    expect(placeRelicError(g.state, 0, second.id)).toBe(
      `${city.name} already keeps a relic`,
    );
  });
  ...
  it("is refused every act when it is not an apostle, in the piece’s own words", () => {
    const { g } = ready();
    const worker = createUnit(g.state, 0, "worker", 5, 5);
    expect(healAdjacentError(g.state, 0, worker.id)).toBe(
      "A Worker is no apostle",
    );
    expect(placeRelicError(g.state, 0, worker.id)).toBe(
      "A Worker is no apostle",
    );
  });
});
...
function landBeside(
  ...
  for (const tile of mapRange(
    ...
    if (avoid.some((hex) => hex.col === tile.col && hex.row === tile.row))
      continue;
...
function faith(
  state: GameState,
  playerId: number,
  god: BeliefId = "keeperOfTheHearth",
) {
...
function simSource(file: string): string {
  ...
  expect(`${file} readable`).toBe(
    key === undefined ? `${file} missing` : `${file} readable`,
  );
...
describe("the religion v2 table", () => {
  it("gives every follower and enhancer belief words, or says what is missing", () => {
    for (const id of [...FOLLOWER_BELIEF_IDS, ...ENHANCER_BELIEF_IDS]) {
      ...
      if (beliefDef(id).effects.length === 0) {
        expect(
          clauses.some((clause) => clause.deferred),
          id,
        ).toBe(true);
      }
    }
  ...
  it("keeps one id space across all three pools", () => {
    ...
    expect(new Set(all).size).toBe(all.length);
    for (const id of all)
      expect(anyCardDef(id).name.length, id).toBeGreaterThan(0);
    ...
    for (const id of FOLLOWER_BELIEF_IDS)
      expect(isPantheonBeliefId(id), id).toBe(false);
  });
  ...
  it("reads both new shapes from a live row — the register", () => {
    ...
    for (const effect of buildingDef("hagiaSophia").effects ?? [])
  ...
  it("holds enough of each pool to fill an offer several times over", () => {
    expect(FOLLOWER_BELIEF_IDS.length).toBeGreaterThanOrEqual(
      RULES.offers.belief * 3,
    );
    expect(ENHANCER_BELIEF_IDS.length).toBeGreaterThanOrEqual(
      RULES.offers.belief * 3,
    );
  });
});
...
describe("The High Temple", () => {
  it("is the node that hands over the prophet, the temple and a third god", () => {
    ...
    expect(techDef("epicPoetry").unlocks.abilities ?? []).toContain(
      "ancestorRites",
    );
    ...
    expect(techDef("philosophy").unlocks.buildings ?? []).not.toContain(
      "temple",
    );
  ...
  it("opens the prophet on its own faith ladder, and never to gold", () => {
    ...
    expect(
      explainPurchaseCost(g.state, 0, city.id, PROPHET, "gold"),
    ).toBeNull();
    ...
    expect(
      explainPurchaseCost(g.state, 0, city.id, PROPHET, "faith")!.total,
    ).toBe(spec.cost);
    player.prophetsPurchased = 1;
    expect(
      explainPurchaseCost(g.state, 0, city.id, PROPHET, "faith")!.total,
    ).toBe(spec.cost! + spec.increment!);
  });
...
describe("founding a religion", () => {
  ...
  it("refuses once the world holds every religion it will", () => {
    ...
    const third = game();
    expect(foundReligionError(g.state, 0)).toBe(
      "Ada has already founded a religion",
    );
    void third;
    const g2 = createGame({
      seed: 3,
    ...
    expect(foundReligionError(g2.state, 2)).toBe(
...
describe("the citizen model", () => {
  ...
  it("converts a road-joined town in eight turns — the tuning, the other lever", () => {
    ...
    for (let col = 7; col <= 11; col++)
      getTileAt(g.state.map, col, 6)!.road = 0;
    expect(
      explainPressure(g.state, target).find((line) => line.source === "Road")
        ?.amount,
...
describe("the pressure ledger", () => {
  it("names every source with its own number, and folds to the total", () => {
    ...
    expect(totals[religion.id]).toBe(
      lines
        .filter((line) => line.religion === religion.id)
  ...
  it("doubles a town’s own faith at its temple and halves everybody else’s", () => {
    ...
    expect(walled[theirs.id]).toBe(
      Math.floor((bare[theirs.id]! * RULES.religion.templeOwnPercent) / 100),
    );
    expect(walled[mine.id]).toBe(
      Math.floor((bare[mine.id]! * RULES.religion.templeForeignPercent) / 100),
    );
    const lines = explainPressure(g.state, target).filter(
      (line) => line.source === "Temple",
    );
  ...
  it("leaves nothing standing behind it — the lump is the whole act", () => {
    ...
    expect(pressureTotals(g.state, target)[religion.id]).toBe(
      RULES.religion.cityStrength,
    );
  });
  ...
  it("does not reach a town one hex past its range", () => {
    ...
    expect(wrappedDistance(g.state.map, here, there)).toBe(
      RULES.religion.bombRange + 1,
    );
    ...
    expect(result.proclaimed?.cities.some((one) => one.cityId === far.id)).toBe(
      false,
    );
  ...
  it("banks on a town that already follows, and caps rather than hoards", () => {
    ...
    expect(target.followers?.[religion.id]).toBe(3);
    expect(target.pressureBank?.[religion.id]).toBe(
      RULES.religion.pressurePerConvert - 1,
    );
    expect(result.proclaimed?.cities).toContainEqual({
      cityId: target.id,
      converted: 0,
...
describe("what a religion pays whom", () => {
  it("pays a follower belief into every following city and into no empire", () => {
    ...
    const paid = (city: City): CardYieldLine | undefined =>
      explainCardCityYields(g.state, city).find((line) =>
        line.source.includes("The Quiet Hours"),
      );
    ...
    for (const seat of [0, 1]) {
      expect(
        liveEffects(g.state, seat).some((entry) =>
          entry.source.includes("The Quiet Hours"),
        ),
        `seat ${seat}`,
      ).toBe(false);
      expect(
        explainCardEmpireYields(g.state, seat).some((line) =>
          line.source.includes("The Quiet Hours"),
        ),
        `seat ${seat}`,
      ).toBe(false);
  ...
  it("pays the four city-local rows into the town that follows", () => {
    ...
    const farmLine = followerCardTileLines(g.state, theirs).find(
      (line) =>
        line.on.test === "improvement" && line.on.improvement === "farm",
    );
    ...
    expect(simSource("yields/hex.ts")).toContain(
    ...
    expect(
      monks(elsewhere, getTileAt(g.state.map, quiet.col, quiet.row)!),
    ).toBe(0);
  });
  ...
  it("pays happiness into the owner of a following town, not into the founder", () => {
    ...
    expect(
      cardHappiness(g.state, 1).some((line) => line.source.includes("Temple")),
    ).toBe(false);
    expect(
      cardHappiness(g.state, 0).some((line) =>
        line.source.includes("Feast Days"),
      ),
    ).toBe(false);
    expect(
      buildingHappiness(g.state, 0, cardBuildingHappiness(g.state, 0)).some(
        (line) => line.source.includes("Feast Days"),
      ),
    ).toBe(false);
  });
  ...
  it("pays the founder’s half to whoever holds the holy city, and moves it on capture", () => {
    ...
    expect(heldReligions(g.state, 0).map((one) => one.id)).toEqual([
      religion.id,
    ]);
    ...
    expect(
      liveEffects(g.state, 0).some((entry) =>
        entry.source.includes("Apostles"),
      ),
    ).toBe(true);
    ...
    expect(
      liveEffects(g.state, 1).some((entry) =>
        entry.source.includes("Apostles"),
      ),
    ).toBe(true);
    expect(
    ...
    expect(
      liveEffects(g.state, 0).some((entry) => entry.source.includes(god)),
    ).toBe(true);
    expect(
      liveEffects(g.state, 1).some((entry) => entry.source.includes(god)),
    ).toBe(false);
  });
  ...
  it("counts the tide five ways, through the beliefs that ask for each count", () => {
    ...
    bumpRevision(g.state);
    expect(
      cardHappiness(g.state, 0).find((line) =>
        line.source.includes("Congregation"),
      )?.amount,
    ).toBe(1);
    ...
    expect(
      explainCardEmpireYields(g.state, 0).find((line) =>
        line.source.includes("Pilgrims' Coin"),
      )?.gold,
    ).toBe(20);
    ...
    expect(
      explainCardEmpireYields(g.state, 0).find((line) =>
        line.source.includes("The Long Prayer"),
      )?.culture,
    ).toBe(2);
    ...
    expect(
      liveEffects(g.state, 1).some((entry) =>
        entry.source.startsWith("Religion"),
      ),
    ).toBe(false);
  });
});
...
describe("the prophet’s four verbs", () => {
  ...
  it("walks one belief ladder: three follower beliefs, then two enhancers", () => {
    ...
    const drawOne = (): void => {
      ...
      expect(
        gainBeliefError(g.state, 0, piece.id),
        String(religion.follower.length),
      ).toBeNull();
      expect(
        applyCommand(g.state, {
          type: "gainBelief",
  ...
  it("replays a prophet’s charges byte for byte", () => {
    ...
    const stones = getTileAt(
      first.state.map,
      religion.holySite!.col,
      religion.holySite!.row,
    )!;
  ...
  it("plants nothing but a holy site, and lets nobody else plant one", () => {
    ...
    expect(improvementError(g.state, augur.id, "farm")).toBe(
      "A augur builds nothing",
    );
  });
...
describe("the inquisitor", () => {
  ...
  it("strips every rival faith’s bank, spares its own, and spends the piece", () => {
    ...
    expect(
      result.ok && result.purged?.cities.map((city) => city.cityId).sort(),
  ...
  it("folds that aura into the forecast on whichever side is standing beside it", () => {
    ...
    expect(guarded.attackerStrength - before).toBe(
      RULES.religion.inquisitorAuraStrength,
    );
    expect(
      guarded.bonuses.some(
        (line) => line.source === "Inquisitor" && line.side === "attacker",
      ),
    ).toBe(true);
  });
});
...
describe("the ratified religion rows", () => {
  it("followingTerritory — The Crusade fights harder among a foreign congregation", () => {
    ...
    const plain = previewCombat(g.state, mine.id, {
      col: target.col,
      row: target.row,
    });
    expect(plain.ok).toBe(true);
    ...
    bumpRevision(g.state);
    const crusading = previewCombat(g.state, mine.id, {
      col: target.col,
      row: target.row,
    });
    ...
    theirs.followers = {};
    const lapsed = previewCombat(g.state, mine.id, {
      col: target.col,
      row: target.row,
    });
  ...
  it("The Crusade presses a lump of faith wherever its soldiers kill", () => {
    ...
    const banked =
      (theirs.pressureBank?.[religion.id] ?? 0) +
      (theirs.followers[religion.id] ?? 0);
  ...
  it("presses nothing for an empire that has founded no religion", () => {
    ...
    applyCombat(g.state, mine.id, { col: target.col, row: target.row });
    expect(
      JSON.stringify({ f: theirs.followers, p: theirs.pressureBank }),
    ).toBe(before);
  });
  ...
  it("The Crusade's line stops at your own towns, which is what foreign means", () => {
    ...
    expect(
      preview.bonuses.some((line) => line.source.includes("The Crusade")),
    ).toBe(false);
  });
  ...
  it("The Stone Hoard pays a mine or a quarry that stands on a seam, and bare rock nothing", () => {
    ...
    const paid = (): { culture: number; faith: number } | undefined => {
      const line = explainTileYield(tile, yieldContextFor(g.state, 0)).find(
        (entry) => entry.source.includes("The Stone Hoard"),
      );
      return line === undefined
        ? undefined
        : { culture: line.culture, faith: line.faith };
    };
  ...
  it("keeps The Living Rock readable after withdrawing it from every pool", () => {
    ...
    expect(beliefPool(g.state, playerById(g.state, 0)!)).not.toContain(
      "theLivingRock",
    );
    ...
    tile.improvement = "mine";
    tile.resource = "iron";
    const paid = explainTileYield(tile, yieldContextFor(g.state, 0)).find(
      (line) => line.source.includes("The Living Rock"),
    );
...
describe("the beliefs balance pass", () => {
  ...
  it("Vineyard Rites pays a plantation and nothing else it works", () => {
    ...
    const paid = (): { food: number; culture: number } | undefined => {
      const line = explainTileYield(tile, yieldContextFor(g.state, 0)).find(
        (entry) => entry.source.includes("Vineyard Rites"),
      );
      return line === undefined
        ? undefined
        : { food: line.food, culture: line.culture };
    };
    delete tile.improvement;
    expect(paid()).toBeUndefined();
    tile.improvement = "farm";
  ...
  it("Cult of Heroes takes its share of the whole trickle, once, and feeds no family", () => {
    ...
    const paying = BUILDING_IDS.find(
      (id) => (buildingDef(id).renown?.perTurn ?? 0) > 0,
    )!;
  ...
  it("Feast Days pays the town once and the temple once, in two ledgers", () => {
    ...
    const asTown = cardHappiness(g.state, 0)
      .filter((line) => line.source.includes("Feast Days"))
      .reduce((sum, line) => sum + line.amount, 0);
    const asShelf = buildingHappiness(
      g.state,
      0,
      cardBuildingHappiness(g.state, 0),
    )
      .filter((line) => line.source.includes("Feast Days"))
      .reduce((sum, line) => sum + line.amount, 0);
    ...
    expect(
      buildingHappiness(g.state, 0, cardBuildingHappiness(g.state, 0)).some(
        (line) =>
          line.source.includes(noTemple.name) &&
          line.source.includes("Feast Days"),
      ),
    ).toBe(false);
  });
  ...
  it("reads Choirs and Tithe Houses off their own rows", () => {
    ...
    const step = (id: "choirs" | "titheHouses"): number => {
      const effect = beliefDef(id).effects.find(
        (entry) => entry.kind === "pays",
      );
      return effect !== undefined && effect.kind === "pays"
        ? (effect.per ?? 1)
        : 1;
    };
    ...
    expect(paid("titheHouses", "gold")).toBe(
      Math.floor(12 / step("titheHouses")),
    );
  });
  ...
  it("shifts Itinerant Preachers’ reach and Ecclesia’s stones off the data", () => {
    ...
    const paid = explainTileYield(stones, yieldContextFor(g.state, 0)).find(
      (line) => line.source.includes("Ecclesia"),
    );
...
describe("the Knights Templar", () => {
  ...
  it("measures no row that awaits a technology, and sells nothing with no horse taught", () => {
    ...
    bare.techsResearched = bare.techsResearched.filter(
      (id) => id !== "theWheel",
    );
    bumpRevision(g.state);
  ...
  it("is opened by the belief and by nothing else — no tree, no queue, no gold", () => {
    ...
    expect(
      purchaseError(shut.g.state, 0, shut.seat.id, TEMPLAR, "faith"),
    ).toContain("not open to");

    const open = order();
    expect(isUnlocked(open.g.state, 0, "unit", "knightsTemplar")).toBe(true);
    expect(
      purchaseError(open.g.state, 0, open.seat.id, TEMPLAR, "faith"),
    ).toBeNull();
    ...
    expect(
      purchaseError(open.g.state, 1, theirs.id, TEMPLAR, "faith"),
    ).not.toBeNull();

    // Bought or not at all: the queue refuses it and so does the treasury.
    expect(
      buildError(open.g.state, 0, "unit", "knightsTemplar"),
    ).not.toBeNull();
    expect(
      purchaseError(open.g.state, 0, open.seat.id, TEMPLAR, "gold"),
    ).toContain("faith");
  });
  ...
  it("is priced as a share of the best horse this empire could raise", () => {
    ...
    expect(dearer.total).toBe(
      Math.floor(
        (foldUnitCost(explainUnitCost(g.state, 0, later)) * share) / 100,
      ),
    );
  });
  ...
  it("is stamped as strong as that horse on the day it is called", () => {
    ...
    expect(
      unitDef("knightsTemplar").combatStrength + unitStampStrength(piece),
    ).toBe(unitDef(horse).combatStrength);
    ...
    learn(g.state, 0, "militantOrders");
    const later = mirrorRowFor(g.state, 0, "knightsTemplar")!;
    expect(unitDef(later).combatStrength).toBeGreaterThan(
    ...
    g.state.turn += 1;
    purchaseItemAt(g.state, player, seat, TEMPLAR, "faith");
    const second = g.state.units.filter(
      (unit) => unit.type === "knightsTemplar",
    )[1]!;
    expect(
      unitDef("knightsTemplar").combatStrength + unitStampStrength(second),
    ).toBe(unitDef(later).combatStrength);
  });

  it("fights better where the faith is kept, and lends that to nobody else", () => {
    const { g, seat, player } = order();
    purchaseItemAt(g.state, player, seat, TEMPLAR, "faith");
    const templar = g.state.units.find(
      (unit) => unit.type === "knightsTemplar",
    )!;
...
describe("the faith ladder as a reading", () => {
  ...
  it("says nothing about turns when nothing is gathering, and says so when it is enough", () => {
    ...
    expect(nextRungWords(explainNextRung(g.state, 0, 3))).toContain(
      "enough is gathered",
    );
  });
  ...
  it("names the technology that opens the next place, and calls a full pantheon full", () => {
    ...
    expect(nextRungWords(shut)).toBe(
      `${techDef("theHighTemple").name} opens the next place`,
    );
    ...
    bumpRevision(g.state);
    expect(nextRungWords(explainNextRung(g.state, 0, 5))).toBe(
...
describe("the standing stones hand over a god, not an agent", () => {
  it("opens a consecration the pick pays nothing for", () => {
    ...
    const realised = realiseItem(g.state, city, {
      kind: "building",
      id: "stonehenge",
    });
...
describe("the second beliefs pass", () => {
  ...
  it("Herd Gods pays a pasture and nothing else the town works", () => {
    ...
    const paid = (): { production: number; faith: number } | undefined => {
      const line = explainTileYield(tile, yieldContextFor(g.state, 0)).find(
        (entry) => entry.source.includes("Herd Gods"),
      );
      return line === undefined
        ? undefined
        : { production: line.production, faith: line.faith };
    };
    delete tile.improvement;
    expect(paid()).toBeUndefined();
    tile.improvement = "farm";
  ...
  it("opens a faith house for the owner of a following town, and for nobody else", () => {
    ...
    expect(purchaseError(g.state, 0, mine.id, MOSQUE, "faith")).not.toContain(
      "not open",
    );
    expect(purchaseError(g.state, 1, theirs.id, MOSQUE, "faith")).not.toContain(
      "not open",
    );
  ...
  it("sells a faith house out of the faith bank alone, at the ordinary price", () => {
    ...
    expect(price.total).toBe(
      Math.floor(hammers * RULES.production.faithPerHammer),
    );
    // Rule 5, for a price: the fold of the printed lines *is* the total.
    expect(price.lines.reduce((sum, line) => sum + line.amount, 0)).toBe(
      price.total,
    );

    // And it is never queued: `purchaseOnly` is the marker, `buildError` the
    // sentence.
    expect(buildError(g.state, 0, "building", "mosque")).toBe(
      "Mosque is not built — it is bought",
    );
  ...
  it("reads the follows gate in one place, and the town-scale unlock in one", () => {
    ...
    const asked = ["purchase.ts", "tech.ts", "cities.ts", "commands.ts"].filter(
      (file) => /\bcityBeliefUnlocksBuilding\(/.test(simSource(file)),
    );
  ...
  it("keeps the world-scale register whole on both sides", () => {
    ...
    for (const count of family)
      expect(WORLD_SCALE_COUNTS, count).toContain(count);
  ...
  it("prints the new rows in the words the worksheet ratified", () => {
    const said = (id: string): string[] =>
      describeCard(id as never).map((c) => stripRefs(c.text));
    expect(said("herdGods")).toEqual([
      "+1 production, +1 faith on every hex with a Pasture",
    ]);
    ...
    for (const [belief, house] of [
      ...
      expect(clause, belief).toContain(
        "it is bought with faith and never built",
      );
      expect(clause, belief).toContain(
