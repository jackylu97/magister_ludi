...
describe('the calendar', () => {
  ...
  it('is on the state before a piece is placed', () => {
    ...
    expect(state.census.nextTurn).toBeLessThanOrEqual(RULES.game.startingTurn + RULES.census.max);
  });
  ...
  it('is a seed: two seeds are two fixed lists of turns', () => {
    ...
    expect(censusTurns(newGame(config({ seed: 7 })), 3)).toEqual([15, 31, 46]);
    expect(censusTurns(newGame(config({ seed: 91 })), 3)).toEqual([16, 31, 45]);
  });
...
describe('the ranking', () => {
  ...
  it('is highest first, and a tie goes to the lower seat', () => {
    ...
    expect(censusRanking(state, 'technologies').map((row) => row.playerId)).toEqual([1, 0]);
  ...
  it('names nobody on a page of noughts', () => {
    ...
    for (const player of state.players) {
      expect(player.triumphs.some((earned) => earned.id === 'censusLeader')).toBe(false);
    }
  });
...
describe('the leader’s Triumph', () => {
  ...
  it('pays the head of the ranking through the one renown seam', () => {
    ...
    state.units = [];
    bumpRevision(state);
    state.players[0]!.techsResearched = ['agriculture', 'earthenware', 'mining'];
    ...
    const line = lines.find((row) => row.source === `Triumph · ${triumphDef('censusLeader').name}`);
    ...
    expect(state.players[0]!.triumphs.filter((earned) => earned.id === 'censusLeader').length).toBe(
      1,
    );
...
describe('the phase', () => {
  it('runs directly after the wagers and directly before the beads', () => {
...
describe('the blocker', () => {
  ...
  it('refuses a second dismissal without moving a byte', () => {
    ...
    runCensus(state);
    state.turn += 1;
    expect(applyCommand(state, { type: 'dismissCensus', playerId: 0 }).ok).toBe(true);

    const before = JSON.stringify(state);
    const again = applyCommand(state, { type: 'dismissCensus', playerId: 0 });
    expect(again.ok).toBe(false);
    expect(JSON.stringify(state)).toBe(before);
    expect(dismissCensusError(state, 0)).toBe('You have already read this census');
  });

  it('refuses a dismissal in a world nobody has counted', () => {
    const state = newGame(config());
    const before = JSON.stringify(state);
    expect(applyCommand(state, { type: 'dismissCensus', playerId: 0 }).ok).toBe(false);
    expect(JSON.stringify(state)).toBe(before);
  });
