  ...
  it('keeps sites apart, on walkable land, and off resources', () => {
    ...
    for (let i = 0; i < sites.length; i++) {
      ...
      for (let j = i + 1; j < sites.length; j++) {
        ...
        expect(
          wrappedDistance(map, tileHex(tile), tileHex(other)),
        ).toBeGreaterThanOrEqual(PLACEMENT.minDistanceApart);
      }
    }
  });

  it('places both kinds, in the numbers the budgets ask for', () => {
    ...
    // The budgets are a *ceiling* — spacing thins them — so the assertion is
    // one-sided by design. See `discoveryPlacement.ts`.
    let land = 0;
    for (const tile of map.tiles) if (!isWaterTerrain(tile.terrain)) land += 1;
    expect(ruins).toBeLessThanOrEqual(Math.round((land / 1000) * PLACEMENT.ruinsPerThousandLand));
    expect(villages).toBeLessThanOrEqual(
      Math.round((land / 1000) * PLACEMENT.villagesPerThousandLand),
    );
  });
...
describe('a blank world', () => {
  it('scatters nothing on a map with no land', () => {
    ...
    placeDiscoveries(map, makeRng(1));
