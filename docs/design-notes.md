...
## Entry LXIX — the arena page (2026-09-04)

The eighth root page (`arena.html`), and the surface the bot's numbers get
tuned on: the whole of `data/ai.json` as a panel **generated by walking the
sheet** — one control per leaf, in the file's own order, grouped by block, with
the age-banded rows as a box per age — so a knob added to the JSON appears
there with no page edit, which is the ruling's own condition (*"with all
further bot changes adding to this page"*). Nothing on the page names a knob;
the test pins both halves (no name as a literal in the builder, every block and
leaf of the current sheet on the panel). Edits become a **sparse override
sheet** shaped exactly like a persona and folded UNDER each seat's persona by
one new seam in `aiConfig.ts` (`setAiTuning`/`withAiTuning`) — untuned it is
identity, so `aiConfigFor(undefined)` is still the very object it was, and a
sheet never reaches a save: a tuned bot emits different commands and *those*
