...
| **X1e** ✅ | **Potential, read off the register of intent** — **BUILT** 2026-09-09 (`docs/bot-priorities.md`, "Batch X1e as shipped"); the user's ruling on flags item (ggg): *“+1 science on libraries is good even if you don't have libraries built yet … a human will take a suboptimal coastal spot over a slightly better inland spot if they suspect fishing boats later”* | `promisedBuildings` (`value.ts`) reads every building step's `copies` off `ValueContext.chains` at `delayDiscount(copy.delay)`, and three arms take it — `buildingYieldPercent` and `pays` at `basis: 'mirror'` fold the same row-for-row arithmetic over the copies, and `townsAdmitting` counts a town the scope would admit with the copy standing (`cityScopeAdmits` of `townHolding`), the wonder idiom kept as a **floor**. `reachableTechs` (`plan.ts`) walks the tree once a sitting — held at nought, on-plan at `turnsUntilPlanned`, off-plan at its own `researchExpansion` road, absent past `priorities.horizonTurns` — and feeds both `plannedRiderTerms` (which asked the declared plan alone) and `reachableGroundOn`, which prices each of `explainSite`'s counted ring hexes at the best improvement a reachable node would open on it, discounted at max(landing, the citizen's arrival). The gate is the simulation's `improvementErrorAt` asked of a **state copy** whose seat holds the reachable nodes and whose unclaimed hexes belong to a phantom town off the map; the yield delta is read through the seat's **own** eyes, so a seam it cannot name still pays nothing. Memos: the promise list, the tech landings, the hypothetical board, and the promises per hex — all `WeakMap`-keyed on the context. **One knob retired**: `site.coastBonus` | **met on the yields row, not on the coastal clause.** t100 probe (eight seeds, 16 seats, main measured in the same process shape): **no column down beyond one standard error** — cities 6.44 → 6.31, citizens 44.2 → 44.9, buildings 19.4 → **20.0**, food 142 → 141, production 73.8 → **75.7**, gold 27.8 → **33.0**, science 56.4 → **58.0**, culture 55.4 → **62.5**, faith 18.0 → 17.8, treasury 320 → 326, techs 23.7 → 22.9 (−0.87 SE), happiness +1.3 → +0.8; ms/turn **+1.9%**. **The coastal clause failed and was isolated**: towns within one hex of water 3.25 → 2.81 and towns with a boat seam in the ring 1.69 → 1.44, both inside one SE and neither up. With the site half shut they read 2.75 and 1.44 (the promise moves them by nothing); with the retired `coastBonus` restored beside the whole batch, 3.00 and 1.56 (the flat prior is under half the drop). The rest is the promise **tilting toward land** — a plain coast hex takes no improvement while every grassland takes a farm and every hill a mine. Restoring the prior costs production (68.0) and science (53.1), so the retirement ships as ruled and **what a coast is worth beyond its hexes — a harbour's routes, a lighthouse's food — is priced nowhere in the settle table**, which is the open question the prior stood in front of. **The batch's own finding**: the first cut ranked the ring by `worth + promise` and cost four buildings, eight science and thirteen culture a seat, because the rank is a guess at which hex the *citizen* takes and a citizen takes the best hex **as it lies** (`yieldScore`) — ranking by the promise stood citizens on bare hills and stalled the growth curve that decides how many hexes are counted at all |
| **X12** ✅ | **Projects as lumps, the faith rate priced, hammers as time** — **BUILT** 2026-09-09 (`docs/bot-priorities.md`, "Batch X12 as shipped"); the user's rulings on flags item (ggg), read off seed 1 on the landed tree | (1) `explainProjectRow` (`value.ts`) folds a `ProjectPayout` through **`explainLump`** — a conversion pays once per completion, not every turn — reads all four keys of the payout (Pageants' culture was worth nothing at all), and yields its bead to `raceTerm` while the race is live; `readNodeGifts` (`chain.ts`) prices a node's conversion projects through that same fold at the node's own landing, and **`research.projectValue` retires**. Beside it, `explainBuildingRow`'s **completion** lines become lumps through `lumpOfPoints` — `renown.onComplete`, the glass bead, the free technology — while its per-turn lines stay rates; a free piece stays a standing thing. (2) `faithPrice` (`value.ts`), `sciencePrice`'s twin: `weights.faith` + Σ over `ctx.wants.faith` of `want.worth × (discount(delay − drop) − discount(delay))`, `delay = want.delay + (price − pool) ÷ rate` with `faithPerTurn` **floored at one**, capped at `priceBandHigh` × the table. `voiceWeight(ctx, 'faith')` hands out that **rate** price; `ctx.prices.faith` stays the **stock** price the spend arms read. (3) `techGoalTable`'s incumbent margin is **symmetric** — a negative plan is *divided* by `switchMargin`, printed either way — and the `explainLump` subtraction of a step's hammers leaves `techChain` and `townChainShare` (`chain.ts`) for a zero-valued printed line, by X1b's own argument: production is always spent on something, and each copy already waits on its own town's cursor. `hammerPrice` and `chainCompression` survive as **derivatives of that wait**, and say so | **met on every column and better than the acceptance asked.** t100 probe (eight seeds, 16 seats, main measured in the same process shape): buildings **19.4 → 36.1**, science **55.3 → 100.1**, techs **22.7 → 27.1**, culture **60.6 → 86.7**, production **74.5 → 95.9**, gold **33.5 → 50.3**, treasury **336 → 506**, faith **17.9 → 19.9**, happiness **+2.0 → +4.3**, citizens 44.1 → 45.2; cities 6.31 → 6.00 (−0.7 SE, the one column down and inside noise); **pantheons founded by t100 0.88 → 1.00 of 16 seats**; ms/turn −5%. Seed 1's capital builds a **Shrine t17–24** (it priced 3.5 against the Worker's 3.0 and main never built one at all — main read Shrine 10.0 against Worker 26.1 at t30), then Granary t34 · Monument t44 · Library t55, and queues **no conversion project in 120 turns** where main queued fourteen and made the Library wait from Writing at t37 to t88. The t108–118 wobble (Satrapies ↔ Daughter Cities four times in four turns) is gone: two isolated single re-aims remain between t100 and t120 against main's five. **The batch's own finding**: with no hammer lump left, a half-paid chain's remaining worth **falls** as it is paid rather than rising — the sunk-cost story `chain.ts` rule 1 claimed was an artefact of a flat-bag payoff against a per-copy cost, and what defends a plan in flight is now the symmetric margin alone |
