# The Wager — the worksheet, archived (2026-09-09)

**This file is history.** The live reference — the rules in current-state form,
the whole deck as one editable table, and the malice deck — is `docs/wager.md`,
and it is the only one of the two that is sync-tested against the data. What
follows is the worksheet the wager was designed on, kept whole: the user's own
words, the cut/keep list, the bench measurements every bar was derived on, the
rulings as they were resolved, and the parts proposed but not built (the Horde,
the census). Figures here are the ones the bars were *first* cut at and may no
...
**Two of these bars no longer stand** (the user's ruling of 2026-09-09,
`docs/flags.md` (nnn)): The Academies was cut to 1 500 · 5 000 · 7 000 and The
Contented Realm to 200 · 250 · 300 — both well below 1.5× the bench mean above.
▢ Pending the user's own balance pass. The live figures are `docs/wager.md`'s
table; the figures in this file are the bench they were first cut on.
...
*Archive note (2026-09-09):* the worksheet was frozen before batches **G3** (the malice deck, schema 105) and **C1** (the census, schema 106) landed; their "landed" marks and the census's BUILT paragraph are on `docs/flags.md` and in the live `docs/wager.md`'s rules, not here.
