...
## 1. The chain, in words
    ...
    the walk                  liveEffects(state, seat)  · statecraft.ts:1423
       ...
       │                      → foldTileLines (base/override replace, add sums)
       │
       ├─ the town's flats    explainCity  · cities.ts:3158
       │                        centre → worked hexes → explainCardCityYields →
       │                        cityResourceYields → specialists → route lines →
       │                        palace → buildings → explainCardBuildingYields →
       ...
       │                      → foldCityStages → applyStages  · modifiers.ts:152
    ...
    the banks                 collectYields  · cities.ts:3836
       ...
       │
       └─ the bot             foldEmpireRates → deckReading → deckMargin
...
* **`foldEmpireRates` is not `collectYields`.** It sums steps 1, 3, 4 and 5
...
* **Three separate reimplementations of `explainCity`'s summands exist** — the
...
| 3 | **W/P** | `topBar.ts:134-184`, `ledgerScreen.ts` `explainLedger` | Neither adds `senderRouteYields`, which `collectYields` banks at `cities.ts:3931-3937` (gold + science + culture of every **outbound foreign** route) | Add the fold beside `empireResourceYields` in both | one line each | **yes** — read against `collectYields`; `test/ui/ledgerScreen.test.ts:144` pins the two surfaces to *each other*, so both drift together | **fixed** — both surfaces add `senderRouteYields`, and the new pin is against the **banked** delta rather than against each other |
| 4 | **P** | `ledgerScreen.ts:311-350` | `cityFlatsByClass` is a declared mirror of `explainCity` and has drifted three ways: (a) it never walks `explainCardBuildingYields`; (b) `:316` floors `population × CITIES.sciencePerPop`; (c) `:337` floors `population × entry.sciencePerPop` | Walk the eleventh summand; drop both floors (batch X made `explainCity:3173` and `:3266` exact) | one arm | **yes** — `sciencePerPop` is `0.5` (rules) / library `0.5` · monastery `0.25` · university `0.75`; seven live `buildingYieldPercent` Orders exist. The pin at `test/ui/ledgerScreen.test.ts:152` passes only because the bench has neither | **fixed** — the mirror walks `explainCardBuildingYields` and both floors are gone; the bench grew a `buildingYieldPercent` Order and an odd-population town |
| 5 | **B** | `value.ts:1456` (`deckReading`), via `cities.ts:4026` | `foldEmpireRates` omits `explainCardEmpireYields`, so the bot's margin cannot see **any** empire-scale card line | Add the empire card fold to the reading the bot takes (not to `foldEmpireRates`, which must stay conversion-safe) | one arm | **yes** — `enhancerBeliefs/apostles` is `effectAmplifier{founderTrickle}`, `foldReadsAmplifier` routes it to the margin, and `founderTrickle` pays `where:'empire'` → margin is exactly 0 | left — batch **H2**'s fence (`src/ai/`) |
...
| 9 | **W** | `cities.ts:600-601, 645` | `paidSoFar` memoises `foldTileLines(list)` **inside** the loop that pushes card lines, so a `TileCondition{test:'yields'}` may or may not see an earlier card's add depending on walk order | Hoist the fold before the card loop, as the docblock claims | one line | read-confirmed; deterministic, so no replay risk — but the printed rule and the code differ | left — not in H1's queue; deterministic, so nothing replays differently. The printed rule and the code still differ |
| 10 | **W** | `cities.ts:3961` | `explainEmpireCardYields` runs **after** the treasury has been credited in the same phase, so `foldEmpireRates` re-prices every town against a debt state that may have flipped mid-phase; a `rateConversion` then reads science/culture the phase did not bank | Take the rate reading before step 2 banks, or freeze `empirePercents` for the phase | one arm | read-confirmed; bites only on the turn an empire crosses zero | left — not in H1's queue; one-turn window on the turn an empire crosses zero |
...
1. **Finding 1** — a live Order (Ledger-Keepers) pays nothing at all on a
   ...
   `explainCity`'s three-voice fold.
...
| a floor surviving batch X on a yield line | `explainCity:3173/3266`, `explainCardBuildingYields:2440`, `explainTileYield:743/762`, `growthSurplus:3449`, `borderGrowth:3589`, `amplify` `routeYields.ts:415`, `explainCityRenown` `renown.ts:216`, `amplifyTrickle` `statecraft.ts:1694` | all exact; the survivors are findings 4, 12 and the intentional integer prices (`explainUnitCost`, `explainPurchaseCost`, `explainTilePurchase`, road maintenance, meter luxury lines) |
| a scope ignored | `cityScopeAdmits` reached from `explainCardCityYields:3273`, `explainCardPercentYields:3977`, `cardProduction:4039`, `cardBuildingPercents:3551`, `cardYieldConversions:3498`, `cardRulePercent:4152`, `scopedCardTileLines:3847`, `cardCityRenownShares:5838`; `scopeAdmits` `resourceEffects.ts:241` for `coastal`/`owner`/`capital` | clean; findings 15/16 are the two readers that cannot ask |
...
| `appliedLast` that isn't last | `cardBuildingPercents` returns `[...ordinary, ...last]` (`statecraft.ts:3563`); `explainCardBuildingYields` runs `for (const pass of [false, true])` and takes the ordinary shares off `base`, the last ones off `raised` (`cities.ts:2432-2451`) | clean |
...
| determinism | no `Math.random`, `Date` or clock anywhere in `src/sim/` or `src/ai/`; `Map`/`Set` appear only as indices and unions (`explainTileYield:643`, `explainCardBuildingYields:2418`, `endpointLuxuryCount:574`, the impact `Bucket`) with output ordered by an array | clean |
...
| `nominalRate` = `unknownEffect × nominalCount` | 6 | `nominalRate` | "the per-turn yield a percentage is a percentage of" | the seat's own `foldEmpireRates` per voice ÷ cities | KEPT — the percentage arms were not in H2's fence |
...
* **the margin's hole** (finding 5) — **CLOSED by batch H2**. `deckReading` now
  reads `marginRates`, which is `foldEmpireRates` **plus**
  `explainEmpireCardYields`. The sender's foreign routes and the treasury's four
  lines were already inside the base reading (`foldEmpireRates`, `cities.ts:4111-4118`),
  so the card empire lines were the whole of the hole. It is built in `value.ts`
  rather than added to `foldEmpireRates`, deliberately: that function's meaning
...
| city panel · flats | `cityPanel.ts:1560-1621` | **same fold** — prints the very lists `explainCity` folds | drops the route line's science/culture (`:1467`) — cosmetic half of finding 1 |
| city panel · stages | `cityPanel.ts:1629` → `foldCityStages` | **same fold** | |
...
| build list preview | `explainBuildingPreview` `cities.ts:2566` | **same fold** — a ghost diff of `foldCity` with a named reconciliation line | model preview; nothing reimplemented |
| card stamp | `explainCardImpact` `cardImpact.ts:649` | **same fold** — two ghosts of `foldCity`, `explainEmpireCardYields`, `empireResourceYields`, `explainEmpireGold`, `happinessOf`, `authorityOf` | best-built preview in the tree; misses `senderRouteYields` for the same reason finding 3 does |
| top bar headline | `civYields` `topBar.ts:134` | **beside it** | re-sums `collectYields`' six steps and omits step 4 — finding 3 |
| Ledger band 1 · totals | `explainLedger` `ledgerScreen.ts` | **beside it** | same omission; pinned only against `civYields` |
...
| great-person face | `actGainOf` `greatPeople.ts:815` → `foldEmpireRates` | **same fold** | inherits finding 5's hole |
