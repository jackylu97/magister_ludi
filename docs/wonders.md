...
| **Stonehenge** | Stonecraft → Standing Stones | 130 | +3🕯 | **+1 pantheon slot** **(new shape: a slot grant on a card — one field, `pantheonSlots: 1`, read where Divination's two are)** · stone-resource tiles +1🕯 | religion's anti-spam structure; quarries |
| **The Pyramids** | Stonecraft → Ancestor Rites | 150 | +2⚙ | **workers +1 charge** (`unitStat`) · the chop windfall +25% (`windfallRider`) | charge workers, the chop |
...
| **The Walls of Uruk** | Bronzeworking → Kingship | 140 | +2🎵 | **capital +10 defense** (`cityStat`) · units within the capital's borders +2 combat (`combatLine`) · +1 authority capacity | palisade, the garrison line, authority |
| **The Great Ziggurat** | Letters → The High Temple | 160 | +2🔬 +2🕯 | **augurs cost −25%** **(new shape: a purchase-price line — `purchaseRider`, one line in `explainPurchaseCost`'s bank; the same line the M9 gold discounts would use)** · shrines +1🔬 | the augur ladder, the shrine's science |
...
| **The Temple of Artemis** | Husbandry → Wayfinding | 140 | +2🌾 +1🕯 | **camps and pastures +1🌾** · +1 happiness per improved *bonus* resource, max +4 (`countScaled`) | the improvement-resource rows, happiness supply |
...
| **The Circus Maximus** | Construction | 300 | — | **+4 happiness** (`happiness`) · +1 happiness per barracks, max +3 (`countScaled`) | Funeral Games' supply side, the tier at +5/+10 |
...
| **The Theatre of Dionysus** | Drama | 320 | +3🎵 | **a free Statecraft draft** on completion (`settleCultureWindfall` fills the basket) · amphitheaters +1🎵 | the culture bucket |
...
| **Hagia Sophia** | Theology | 480 | +3🕯 | **a free augur** on completion (`realiseItem`) · temples +1🕯 +1🎵 | the augur ladder (a free one skips a rung) |
...
| **The Alhambra** | Chivalry | 460 | +3🎵 | **mounted units +1 combat** (`combatLine`) · +1 happiness · units built here start with the garrison bonus | the mounted line, Tourney Ground |
...
| **The Great Mosque of Djenné** | Theology | 460 | +3🕯 | **augurs +1 rite charge** (`unitStat` on the augur's `chargesLeft` at creation) · +1 pantheon slot **(new shape, shared with Stonehenge)** | rites, slots |
| **The Water Clock of Su Song** | Machinery | 440 | +3🔬 | **Scholarship 20⚙ → 8🔬** (`rateConversion`) · +1 authority capacity | the projects |
| **Notre-Dame** | Theology | 470 | +2🎵 | **+1 happiness per temple**, max +5 (`countScaled`) · cathedrals +2🎵 | happiness supply, the temple count |
| **The Forbidden City** | Education | 500 | +2🎵 | **+4 authority capacity** · Statecraft: one more Order office **(new shape: an office grant — the government layout plus one slot of a stated kind)** | the authority meter, the offices |
