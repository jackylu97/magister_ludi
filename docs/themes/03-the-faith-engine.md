# 03 · The Faith Engine
Faith-per-turn routed through converters; augurs and rites spend the rest.
Wins by: the Opus faith pillar · The Cathedral of the Age · The Hierophant.

## Æra I
- Tech:
  - Divination — unlocks Augur, Shrine, The Oracle, Rite of the Harvest, Recasting the Omens
- Orders:
  - First Rites — +2 faith in your capital.
  - Fire-Keepers — +1 faith in your capital, and +1 happiness there.
- Doctrines: —
- Religion:
  - the pantheon drafts
  - Rite of the Harvest — augur rite — +1 population
  - Omen Reading — augur rite — +15 science
- Great people: —
- Wonders:
  - Stonehenge — +1 pantheon slot · +1 faith on every hex carrying Stone or Marble · on completion, an Augur joins you
  - The Oracle — +1 faith in every city with The Oracle · +1 card in every Statecraft draft · +1 card in every Doctrine draft
- Deeds:
  - The First Faith — The first religion founded in the world.

## Æra II
- Tech:
  - The High Temple — unlocks Prophet, Temple, The Preaching
  - Ancestor Rites — unlocks Ancestor Rites, Monument
- Orders:
  - Pilgrim Roads — +1 faith for every 3 citizens in your capital · +1 happiness for each 50 banked faith (at most +5).
  - Lamplighters — +1 culture for each 5 faith you gain per turn.
  - The Pilgrim's Purse — +5 faith in every city standing beside a holy site.
  - Wayside Shrines — +1 faith in every city.
  - Census of Souls — +1 faith for each citizen in your capital.
  - The Choir — +1 culture and +1 happiness in every city with a Temple.
- Doctrines:
  - The Great Litany — +1 culture for each 3 faith you gain per turn.
  - The Tithe — +1 gold for every 2 faith you gain per turn.
  - Divine Inspiration — +1% science and +1% culture for each 200 banked faith.
- Religion:
  - founding
  - follower/enhancer drafts
  - The Preaching — augur rite — lump: {'range': 4, 'amount': 20}
  - Consecration of the Bounds — augur rite — +15 borderCulture
- Great people: the prophet (called with faith)
- Wonders:
  - The Great Ziggurat — religious units cost −25% to buy · +1 science in every city with a Shrine
- Deeds:
  - The Hierophant — Spend a great sum of faith on augurs and prophets across the game.
  - The Tithe — Gather a great sum of gold from tithes.

## Æra III
- Tech: **Theology — moved to Æra III by user ruling (2026-09-01), cost 600** — brings the Cathedral (Entry LV: cost 340, +3 happiness, contribute verb, five consecrations), the Monastery, and the age's faith wonders (Hagia Sophia, Angkor Wat, the Great Mosque, Chichen Itza, Notre-Dame). Theology is also the tech that opens the **enhancer pool** (user: important to have earlier too) — the whole enhancing act now lives in Æra III.
- Orders:
  - Mandate of Heaven — The science and culture your happy cities pay rises from 10% to 15% · +1 happiness for each 200 banked faith.
  - The Saints' Fields *(user)* — great-person improvements pay +3 faith.
  - The Illuminated Texts *(user)* — spending faith pays a third of it back as science.
- Doctrines:
  - Cuius Regio *(rework proposal)* — every city you own immediately follows your religion, and foreign pressure on your cities is halved. ("Whose realm, his religion" — the ruler's faith IS the land's.)
  - Holy War — Permanent war and +2 combat strength against empires of a different majority religion · your cities cannot be converted.
- Religion:
  - enhancers' III act
  - Recasting the Omens — augur rite
- Great people: 
- Fill in some ideas here, go wild
- Wonders:
  - Hagia Sophia — +1 culture, +1 faith in every city with a Temple · spreads your religion +4 faith to every city within 8 hexes · on completion, a Prophet joins you
  - Angkor Wat — faith buildings and improvements (holy sites) in this city gain +2 food. gain +1 faith for every 3 population in this city.
  - The Great Mosque of Djenné — religious pressure +20% for your religion (does nothing if you didn't found one). +1 follower belief.
- Deeds:
  - The Cathedral of the Age — A race. Needs a cathedral in every city. The first empire to finish is sent a prophet.

## Æra IV
- Tech: The Holy Office (below) is the age's node — already traditional format (unit + building unlocks). The observatory cloister is *re-ruled a BUILDING* — its "+10% science with temple and university" becomes the building's own effect, unlocked by the Holy Office — so the tree stays neutral.
  - The Holy Office *(user's effects, named)* — unlocks the Inquisitor (strips a lump of every rival religion's pressure from cities within 5 hexes · +2 combat strength to adjacent units) and **the Reliquary** — RULED merged (2026-09-02): the tier-4 faith building IS the Reliquary (+4 happiness · +10% faith in its city · allows purchasing units with faith). The Basilica stays separate behind The Holy See.
- Orders: 
  - The Great Commission *(user)* — your religion presses +50% harder.
  - The Field Chaplains *(user)* — units adjacent to a religious unit heal +5 every turn, whatever they did that turn.
  - The Holy See *(user)* — unlocks the Basilica (1000🕯, only in a city with a holy site): +5🎵 and +5🔬 · +10%⚙ toward buildings and wonders in its city.
  - The Pilgrim Caravans (🐫 crossover) *(user)* — trade routes carry +1 faith per 2 citizens of their origin city.

Wonders:
  - The Pyramid of the Sun *(user)* — its city gains a rite: spend 1000🕯 and 1 population → +10% science and culture empire-wide for 15 turns (never stacks).
  - El Dorado *(user)* — its city pays 50% of its faith income again as gold · +5🕯 and +5💰.

- Doctrines: 
  - The Scholastic Creed *(user)* — cities pay 15% of their faith income again as science and culture.
- Religion: *(planned)* Holy Order built (warrior monks called with faith) + a late faith unit or two
- Buildings: *(planned)* The Reliquary — faith-purchase-only, renown + happiness
- Deeds:
  - The Ecumenical Council — first faith followed by a world majority
  - Opus pillar: faith
