# 06 · The Luxury Table
Luxuries as a collection game — and the wide player's happiness engine, which the
2026-09-01 ruling makes load-bearing: most of the game's happiness supply moves HERE and
into Orders generally.

**Tech (neutral-tree ruling):** none of its own — rides 05's commerce nodes; the map supplies the luxuries.

## Æra I
- Orders:
  - Common Granary — +1 food in every city holding an improved luxury resource.
  - Sumptuary Laws — +1 happiness for each unique luxury.
  - Village Fairs — +1 happiness for each luxury you hold two or more copies of.
- Religion:
  - Pilgrimage — +1 faith for every luxury held by a city that follows.
- Deeds: ▢ ________ (a collection feat: first to hold four kinds?) [remove, no deeds in age 1/2]

## Æra II
- Orders:
  - Provincial Mints — +2 gold for each improved copy of a luxury — duplicates count.
  - The Orchard Tithe — +1 food on every hex carrying a luxury resource.
- Religion:
  - Lord of the Hoard — +1 culture and +1 faith on every hex with a Mine carrying a luxury resource.
- Doctrines: ▢ ________

## Æra III
- Doctrines:
  - The Grand Bazaar (please write a description here of what this does)
- Orders: 
- luxury resources from captured cities supply +50% happiness
- gain +1 happiness from each unique bonus resource improved in your lands (includes great people improvements)
- gain +2 happiness from every unique luxury you've traded to another player.
- Tech: The Bronze Mirror's trinkets *(deferred: a luxury not on the map is a new mechanism)*
- Deeds: none needed for now

## Æra IV
- none needed for now.

## Gaps
No deed anywhere; the happiness budget re-balance lands mostly on this file and 01/07/14.

## The proposal batch — CUT (user, 2026-09-02)
"I don't like the shape of these — this theme has enough support as is. Will know more from
playtesting." The two survivors, parked as maybes:
- Æra IV · The World's Table *(parked)* — +1% to every yield per unique luxury (cap +15%).
- Æra IV · The Groaning Board *(parked, reckoning)* — most unique luxuries at age close.
